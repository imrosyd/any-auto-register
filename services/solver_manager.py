"""Turnstile Solver process management - auto-started on backend launch"""
import subprocess
import sys
import os
import time
...
# Consecutive startup failure count, prevents infinite retry loop
...
def _ensure_camoufox_browser() -> bool:
    ...
    Returns True if ready, False if download failed (network issues etc.). Called before Solver starts.
    First download is ~100MB, subsequent runs use cache.
    """
    try:
    ...
    try:
        ...
        print(f"[Solver] Camoufox browser detection error, still attempting installation: {e}")

    print("[Solver] Camoufox browser not installed, starting download (~100MB, please wait)...")
    try:
        from camoufox.pkgman import CamoufoxFetcher
        CamoufoxFetcher().install()
        print("[Solver] Camoufox browser download complete")
        return True
    except Exception as e:
...
def start():
    ...
    with _lock:
        ...
        # Too many consecutive failures, refuse to retry (manual restart resets the counter)
        if _consecutive_failures >= _MAX_CONSECUTIVE_FAILURES:
            print(f"[Solver] Failed to start {_consecutive_failures} consecutive times, stopping retries. Please investigate and restart manually.")
            return
        ...
        if not _ensure_camoufox_browser():
            ...
            print("[Solver] Camoufox browser unavailable, skipping Solver startup")
            return

        # After PyInstaller packaging, sys.executable points to the backend executable,
        # Use --solver argument to make it use the solver entry point; in source mode use python + start.py
        ...
        for _ in range(30):
            ...
            if _proc.poll() is not None:
                ...
                try:
                    ...
                    pass
                _consecutive_failures += 1
                _last_failure_reason = stderr_msg or f"Process exited code={_proc.returncode}"
                print(f"[Solver] Subprocess exited abnormally code={_proc.returncode} (consecutive failures {_consecutive_failures}/{_MAX_CONSECUTIVE_FAILURES})")
                if stderr_msg:
            ...
            if is_running():
                ...
                _consecutive_failures = 0
                _last_failure_reason = ""
                # Close stderr pipe to avoid buffer full causing subprocess to block
                try:
                    _proc.stderr.close()
                except Exception:
                    pass
        ...
        if is_running():
            ...
            for _ in range(10):
                ...
                if not is_running():
                    break
            if is_running():
                print("[Solver] Warning: port still in use after stop")
            else:
                print("[Solver] Residual processes cleaned up")


def _kill_by_port(port: int):
    """Find and kill the process occupying the port (cross-platform)."""
    import platform
    try:
...
def start_async():
    """Start in a background thread, does not block the main process"""
