...
def finalize_oauth_email(actual_email: str, email_hint: str, platform_name: str) -> str:
    ...
    if actual and hint and actual.lower() != hint.lower():
        raise RuntimeError(
            f"{platform_name} OAuth login email does not match expected: got {actual}, expected {hint}"
        )
    resolved = actual or hint
    if not resolved:
        raise RuntimeError(
            f"{platform_name} OAuth flow did not resolve an email, provide email or oauth_email_hint in the task"
        )
    return resolved


def _detect_running_chrome_cdp(ports: tuple = (9222, 9223, 9224)) -> str:
    """Check whether a local Chrome instance has remote debugging enabled; return CDP URL or empty string."""
...
def _detect_chrome_user_data_dir() -> str:
    """Auto-detect the system Chrome user data directory."""
    import os, sys
...
def _relaunch_chrome_with_debug_port(port: int = 9222) -> bool:
    """macOS: kill Chrome and relaunch with remote debugging port enabled; return True on success."""
...
class OAuthBrowser:
    """Fully automated OAuth browser (supports plain Playwright / Chrome Profile / CDP)."""
    ...
    def __enter__(self):
        ...
        if self.chrome_cdp_url:
            ...
            if cdp_url:
                ...
                self.log("[OAuthBrowser] No system Chrome found, using Playwright Chromium")
    ...
    def auto_select_google_account(self, timeout: int = 15) -> bool:
        ...
        Works in Chrome Profile mode: Google is already logged in, account picker pops up.
        """
        ...
        while time.time() < deadline:
            for page in self.pages():
                ...
                try:
                    ...
                    if el:
                        el.click()
                        self.log("[OAuthBrowser] Google account picker: auto-clicked first account")
                        return True
                except Exception:
                    pass
            time.sleep(0.5)
