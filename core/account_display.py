...
def _build_generic_usage_metrics(overview: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    _append_metric(primary, _metric("usage_total", "Usage Total", overview.get("usage_total"), tone="muted"))
    _append_metric(secondary, _metric("plan_credits", "Plan Credits", overview.get("plan_credits"), tone="muted"))
    _append_metric(secondary, _metric("reset_days", "Days Until Reset", overview.get("days_until_reset"), sub="days", tone="muted"))
...
def build_account_display_summary(
    *,
    ...
    if plan_state and plan_state != "unknown":
        _append_metric(secondary_metrics, _metric("plan_state", "Plan State", plan_state, tone="muted"))
    if checked_at_value:
    ...
    if validity_status == "invalid" or lifecycle_status == "invalid":
        warnings.append({"key": "invalid", "tone": "danger", "message": "Account check returned invalid"})
    if validity_status == "unknown":
        warnings.append({"key": "unknown_validity", "tone": "warning", "message": "Validity check not yet performed"})
