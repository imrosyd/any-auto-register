"""Abstract base for registration identity providers."""
...
IDENTITY_PROVIDER_ALIASES = {
    ...
    "oauth_manual": "oauth_browser",   # backward-compat alias
    "manual_oauth": "oauth_browser",   # backward-compat alias
}
...
class MailboxIdentityProvider(BaseIdentityProvider):
    ...
    def resolve(self, requested_email: Optional[str] = None) -> IdentityMaterial:
        ...
        if not requested_email and not email:
            ...
            raise ValueError(f"{provider_name} did not return a usable email, check mailbox provider configuration or service status")
        if requested_email and email and requested_email != email:
            raise ValueError(f"Requested email {requested_email} does not match current mailbox provider email {email}")
