...
After configuring the Any2API URL and admin password in global settings,
every successful registration will automatically push the account to Any2API.
"""
...
def _get_any2api_config() -> tuple[str, str]:
    """Read Any2API URL and password from global config."""
    try:
...
def push_account_to_any2api(account: Any, *, log_fn=None) -> bool:
    ...
    try:
        if platform == "kiro":
            ...
            log(f"  [Any2API] Platform {platform} does not support auto-push yet")
            return False
