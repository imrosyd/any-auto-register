...
class BasePlatform(ABC):
    ...
    display_name: str = ""
    version: str = "1.0.0"
    # Platform capabilities are provided by DB tables; no business config defaults on class.
    ...
    def __init__(self, config: RegisterConfig = None):
        ...
        if self.config.executor_type not in self.supported_executors:
            raise NotImplementedError(
                f"{self.display_name} does not support executor '{self.config.executor_type}', "
                f"currently supports: {self.supported_executors}"
            )
    ...
    def register(self, email: str = None, password: str = None) -> Account:
        ...
        if (self.config.executor_type or "") in ("headless", "headed"):
            ...
            if adapter is None:
                raise NotImplementedError(f"{self.display_name} does not implement a browser registration adapter")
        ...
        if getattr(identity, "identity_provider", "") == "oauth_browser":
            ...
            if adapter is None:
                raise RuntimeError(
                    f"{self.display_name} only supports oauth_browser in browser mode, use a supported browser executor"
                )
        ...
        if adapter is None:
                raise NotImplementedError(f"{self.display_name} does not implement a protocol mailbox registration adapter")
    ...
    def get_trial_url(self, account: Account) -> Optional[str]:
        """Generate a trial activation link (optional implementation)"""
        return None
    ...
    def get_desktop_state(self) -> dict:
        return {
            "available": False,
            "message": f"{self.display_name or self.name} does not provide desktop app status probing yet",
        }
    ...
    def _resolve_captcha_solver(self) -> str:
        ...
        if self.config.executor_type in {"headless", "headed"}:
            raise RuntimeError("Browser mode has no default captcha provider configured; enable and set one in Settings first")
        raise RuntimeError("Protocol mode has no available captcha provider; enable and configure at least one captcha provider")
    ...
    def _get_identity_provider(self):
        ...
        if mode not in self.supported_identity_modes:
            raise NotImplementedError(
                ...
                f"currently supports: {self.supported_identity_modes}"
            )
    ...
    def _resolve_identity(self, email: str = None, *, require_email: bool = True):
        ...
        if require_email and not identity.email:
            raise ValueError(
                f"{self.display_name} registration did not obtain a usable email, "
                f"provide email or configure a supported identity_provider"
            )
        return identity
