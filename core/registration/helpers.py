...
def build_phone_callbacks(ctx: RegistrationContext, *, service: str | None = None):
    ...
    if not provider_key:
        ctx.log("[SMS] No SMS provider configured (task params/global default/legacy fields all empty), phone_callback=None — registration will fail at add_phone step")
        return None, None
