...
class BrowserRegistrationFlow:
    ...
    def run(self, ctx: RegistrationContext) -> RegistrationResult:
        ...
        if self.adapter.capability.browser_mailbox_requires_mailbox:
            ensure_mailbox_identity(ctx, f"{ctx.platform_display_name} browser mailbox registration depends on a mailbox provider")
        ...
        try:
            ...
            if worker is None or self.adapter.browser_register_runner is None:
                raise RuntimeError(f"{ctx.platform_display_name} does not implement a browser registration adapter")
...
class ProtocolMailboxFlow:
    ...
    def run(self, ctx: RegistrationContext) -> RegistrationResult:
        ...
        if self.adapter.capability.protocol_mailbox_requires_email:
            ensure_identity_email(ctx, f"{ctx.platform_display_name} registration depends on a mailbox provider, no email account available")
        if self.adapter.capability.protocol_mailbox_requires_mailbox:
            ensure_mailbox_identity(ctx, f"{ctx.platform_display_name} registration depends on a mailbox provider, no email account available")
