...
class LocalMicrosoftMailboxPool(BaseMailbox):
    ...
    def _load_pool_text(self) -> str:
        ...
        if not combined.strip():
            raise RuntimeError("Local Microsoft mailbox pool is empty; paste Xinlan common format or provide a file path")
        return combined

    def _entries(self) -> list[LocalMicrosoftMailboxEntry]:
        entries = parse_xinlan_common_rows(self._load_pool_text())
        if not entries:
            raise RuntimeError("Local Microsoft mailbox pool parsed no valid emails")
        return entries
    ...
    def _graph_access_token(self, entry: LocalMicrosoftMailboxEntry) -> str:
        ...
        if response.status_code != 200:
            raise RuntimeError(f"Microsoft refresh_token to access_token exchange failed: HTTP {response.status_code} {response.text[:200]}")
    ...
    def _graph_messages(self, entry: LocalMicrosoftMailboxEntry) -> list[dict]:
        ...
        if response.status_code != 200:
            raise RuntimeError(f"Microsoft Graph read mail failed: HTTP {response.status_code} {response.text[:200]}")
    ...
    def _imap_messages(self, entry: LocalMicrosoftMailboxEntry) -> list[dict]:
        if not entry.imap_ready:
            raise RuntimeError(f"Microsoft mailbox has no usable Graph token or IMAP inbound config: {entry.email}")
