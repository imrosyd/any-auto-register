"""Generic HTTP mailbox driver — all steps described via DB config, zero-code mailbox type addition.

Pipeline:
  auth_steps[]  →  create_email  →  list_emails  →  get_detail
   (optional)      (optional)      (required)      (optional)

Configuration lives in provider_definitions.metadata_json,
user-supplied values live in provider_settings.config_json / auth_json.
"""
...
class GenericHttpMailbox(BaseMailbox):
    ...
    Config sources (merged by priority):
      1. pipeline_config: advanced pipeline (from provider_definitions.metadata_json)
      2. settings: flat user-supplied fields (api_url, list_path, auth_type, …)

    When pipeline_config is empty, the pipeline is auto-built from the flat settings fields.
    """
    ...
    def __init__(
        ...
        # Build final pipeline from metadata pipeline + flat settings
    ...
    def _build_pipeline(self, raw: dict | None) -> dict:
        """Build the final pipeline config by merging metadata pipeline and flat settings."""
        pipeline = deepcopy(raw or {})
        s = self._settings

        # flat settings values act as defaults; metadata values take precedence
        ...
        if self._session is None:
            ...
            if token:
                if auth_type == "bearer":
                    ...
                    # api_key injected as query parameter on every request
                    pass
    ...
    def _execute_step(self, step_config: dict) -> dict | list | None:
        """Execute a single HTTP step and return the response JSON."""
    ...
    def _run_auth(self) -> None:
        """Execute the auth step chain (if any)."""
    ...
    def get_email(self) -> MailboxAccount:
        ...
        if email_mode == "fixed":
            ...
            if not email:
                raise RuntimeError("Generic HTTP mailbox: email_mode=fixed but no email configured")
            self._email = email
        ...
        if not create_step:
            raise RuntimeError("Generic HTTP mailbox: email_mode=generate but no create_email step configured")
        ...
        email = self._vars.get("email", "")
        if not email:
            raise RuntimeError("Generic HTTP mailbox: create_email completed but no email extracted")
        self._email = email
        return MailboxAccount(
            email=email,
            account_id=self._vars.get("account_id", ""),
    ...
    def wait_for_code(
        ...
        list_step = self._pipeline.get("list_emails")
        if not list_step:
            raise RuntimeError("Generic HTTP mailbox: no list_emails step configured")
        ...
        while time.time() - start < timeout:
            try:
                ...
                # If get_detail step exists, fetch individual details
