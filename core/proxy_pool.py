"""Proxy pool - reads proxies from DB, supports round-robin and region-based selection"""
...
class ProxyPool:
    ...
    def get_next(self, region: str = "") -> Optional[str]:
        ...
        Priority:
          ...
          2. Static proxy pool (fixed proxy list in DB)
        """
        # 1. Try dynamic proxy
        try:
        ...
        # 2. Fall back to static proxy pool
    ...
    def report_fail(self, url: str) -> None:
        with Session(engine) as s:
            ...
            if p:
                p.fail_count += 1
                p.last_checked = datetime.now(timezone.utc)
                # Auto-disable after continuous failures
    ...
    def check_all(self) -> dict:
        """Check all proxies for availability"""
        import requests
        with Session(engine) as s:
