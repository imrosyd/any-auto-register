"""Mailbox pool base class - Abstract temporary email/reception service"""
...
# ── Mailbox service default API URLs (centrally maintained, modify here as needed) ──
...
class MailboxAccount:
    email: str
    account_id: str = ""
    extra: dict = None  # Platform-specific extra info
...
class BaseMailbox(ABC):
    ...
    def wait_for_code(self, account: MailboxAccount, keyword: str = "",
        ...
        """Wait for and return a verification code; code_pattern is a custom regex (default matches 6 digits)"""
        ...

    @abstractmethod
    def get_current_ids(self, account: MailboxAccount) -> set:
        """Return the current set of email IDs (used to filter out old emails)"""
        ...

    def wait_for_link(self, account: MailboxAccount, keyword: str = "",
                      timeout: int = 120, before_ids: set = None) -> str:
        """Wait for and return a verification link. Default implementation is provider-specific."""
        raise NotImplementedError(f"{self.__class__.__name__} does not support wait_for_link() yet")


class FallbackMailbox(BaseMailbox):
    """Try multiple providers in order, then use the same provider for receiving after creation."""
    ...
    def get_email(self) -> MailboxAccount:
        ...
        for provider_key, mailbox in self.providers:
            try:
                ...
                print(f"[Mailbox] Successfully using provider: {provider_key} -> {account.email}")
                return account
                ...
                continue
        raise RuntimeError("All mailbox providers failed to create: " + " | ".join(errors))
...
def create_mailbox(provider: str, extra: dict = None, proxy: str = None) -> 'BaseMailbox':
    """Factory method: create the corresponding mailbox instance based on provider"""
    ...
    def wait_for_code(self, account: MailboxAccount, keyword: str = "trae",
        ...
        while time.time() - start < timeout:
            try:
                ...
                pass
            time.sleep(4)
        raise TimeoutError(f"Verification code wait timeout ({timeout}s)")
    ...
    def wait_for_link(self, account: MailboxAccount, keyword: str = "",
        ...
        while time.time() - start < timeout:
            try:
                ...
                pass
            time.sleep(4)
        raise TimeoutError(f"Verification link wait timeout ({timeout}s)")
...
class TempMailWebMailbox(BaseMailbox):
    """Temp-Mail Web API (same as the reference project)."""
    ...
    def _request_json(self, method: str, path: str, *, auth_header: str = "") -> dict | list:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            print(f"[TempMailWeb] {action} got 429, retrying in {wait_seconds:.1f}s ({attempt}/{max_attempts})")
    ...
    def get_email(self) -> MailboxAccount:
        ...
        if not address or not token:
            raise RuntimeError(f"Temp-Mail Web failed to create mailbox: {json.dumps(data, ensure_ascii=False)[:300]}")
        self._accounts[address] = token
        print(f"[TempMailWeb] Generated email: {address}")
        return MailboxAccount(
            email=address,
            account_id=token,
            extra={
        ...
        while time.time() - start < timeout:
            try:
                ...
                for msg in msgs:
                    ...
                    seen.add(mid)
                    # Fetch email detail to get full text
                    try:
...
class MoeMailMailbox(BaseMailbox):
    """MoeMail (sall.cc) email service - auto-register accounts and generate temporary emails"""
    ...
    def _login_with_existing_account(self) -> str:
        ...
        if token:
            ...
            print("[MoeMail] Logged in with manually registered account")
            return token
        raise RuntimeError(
    ...
    # Prefer these domains (better reputation, less likely to be rejected by AWS/Google etc.)
    ...
    def get_email(self) -> MailboxAccount:
        ...
        # Get available domain list, prefer better reputation domains to avoid rejection by AWS etc.
        domain = "sall.cc"
        try:
            with suppress_insecure_request_warning():
            ...
            if all_domains:
                # Filter preferred domains from available ones, order by _PREFERRED_DOMAINS
                preferred = [d for d in self._PREFERRED_DOMAINS if d in all_domains]
                if preferred:
                    domain = random.choice(preferred)
                else:
                    # No preferred domains available, pick randomly from remaining
        ...
        if not email_id:
            ...
            raise RuntimeError(f"MoeMail failed to generate email: {str(generate_error).strip() or f'HTTP {r.status_code}'}")
        if not self._email:
            raise RuntimeError("MoeMail failed to generate email: response missing email")
...
class FreemailMailbox(BaseMailbox):
    ...
    Supports admin token or account/password authentication
    """
        ...
        while time.time() - start < timeout:
            try:
                ...
                for msg in r.json():
                    ...
                    if not mid or mid in seen: continue
                    seen.add(mid)
                    # Try verification_code field directly
...
class DDGEmailMailbox(BaseMailbox):
    """DuckDuckGo Email Protection - generate @duck.com private alias, read verification codes via IMAP from forwarding mailbox"""
    ...
    def get_email(self) -> MailboxAccount:
        ...
        if not address:
            raise RuntimeError(f"DDG Email failed to create alias: {r.text[:200]}")
    ...
    def _imap_search_code(self, alias_email: str, timeout: int, code_pattern: str = None) -> str:
        ...
        if not self.imap_user or not self.imap_pass:
            raise RuntimeError("DDG Email IMAP not configured (ddg_imap_user / ddg_imap_pass), cannot read verification code")
        ...
        while time.time() - start < timeout:
            ...
            try:
                ...
                # First poll: mark all existing emails as seen, only wait for new ones
                ...
                for mid in reversed(ids[-30:]):
                    ...
                    # Check if the email was sent to the alias (DDG forwarding preserves original To)
                    to_addr = str(msg.get("To", "") or "").lower()
                    from_addr = str(msg.get("From", "") or "").lower()
                    subject = str(msg.get("Subject", "") or "")

                    # Only consider emails addressed to alias or from openai/noreply
                    ...
                    if m:
                        ...
                        print(f"[DDG Email] Got verification code via IMAP: {code}")
                        return code
        ...
        raise TimeoutError(f"DDG Email IMAP verification code wait timeout ({timeout}s)")
