"""SMS verification service base class + SMS-Activate / HeroSMS implementation."""
...
class SmsActivateProvider(BaseSmsProvider):
    ...
    def get_number(self, *, service: str, country: str = "") -> SmsActivation:
        ...
        if "NO_NUMBERS" in result:
            raise RuntimeError(f"SMS-Activate: no available numbers (service={service_code}, country={country_id})")
...
def _safe_bool(value, default: bool) -> bool:
    ...
    if isinstance(value, bool):
        return value
    return str(value).strip().lower() not in {"0", "false", "no", "off", "no"}
...
class HeroSmsProvider(BaseSmsProvider):
    ...
    def get_services(self, country: str | int | None = None, lang: str = "cn") -> list:
        ...
        if isinstance(data, dict):
            # Could be {"dr": {"name": "OpenAI", ...}, ...} format
            result = []
    ...
    def get_top_countries(self, service: str | None = None) -> list[dict]:
        """Get countries sorted by price for a given service (includes price and stock).

        Prefers getTopCountriesByServiceRank API, falls back to parsing getPrices full data.
        ...
        # Strategy 1: use getTopCountriesByServiceRank (HeroSMS-specific ranking API)
        ...
        # Strategy 2: parse from getPrices full data
        try:
    ...
    def _parse_top_countries_response(self, data) -> list[dict]:
        ...
        rows = []
        items = data
        # May be nested under data/result key
        if isinstance(data, dict):
    ...
    def get_best_country(self, service: str | None = None, *, min_stock: int = 20, max_price: float = 0) -> str | None:
        """Auto-select best country: lowest price with sufficient stock.
        ...
        Returns:
            Best country ID string, or None (no country available)
        """
        # Whitelist of countries verified to use SMS (not WhatsApp) for OpenAI on HeroSMS/SMSBower
        # OpenAI switched to WhatsApp verification for most countries since 2025
        # Only Thailand is confirmed to still use SMS
        ALLOWED_COUNTRIES = {
            "52",   # Thailand (confirmed SMS)
        }
        ...
        # If nothing meets min_stock, relax to count > 0
        for row in rows:
            country_id = str(row.get("country") or "")
            if country_id not in ALLOWED_COUNTRIES:
                continue
    ...
    def _request_number_raw(self, service: str, country: str) -> dict:
        ...
        # Dynamically get actual price for this country/service, use it as maxPrice
        # This ensures we get a physical number (rather than a virtual one)
        effective_max_price = self.max_price if self.max_price > 0 else 1
        try:
            prices = self.get_prices(service=service, country=country)
            # getPrices returns: {country_id: {service_code: {cost, count}}}
            ...
            if actual_cost is not None:
                ...
                # Use 3x actual cost as maxPrice (leave margin), but not exceeding user config's upper limit
        ...
        # If NO_NUMBERS and maxPrice is below user's upper limit, increase maxPrice and retry
        ...
        try:
            ...
            raise RuntimeError(f"HeroSMS number acquisition failed: V2={v2_error}; V1={exc}") from exc
    ...
    def mark_send_failed(self, activation_id: str, reason: str = "") -> None:
        ...
        if any(keyword in reason_text for keyword in ("limit", "already", "too many", "exceeded", "maximum", "limit", "reached")):
...
class PhoneCallbackController:
    ...
    def __call__(self) -> str:
        ...
        if self.phase == "need_number":
            ...
            # Smart country selection: if auto_select_country is enabled, automatically query best country
            ...
            if auto_select and isinstance(provider, HeroSmsProvider):
                ...
                try:
                    ...
                    if best:
                        ...
                        self.log("No country meeting conditions found, using default config")
                except Exception as exc:
                    self.log(f"Smart country selection failed ({exc}), using default config")

            country_label = effective_country or self.config.get("sms_country") or self.config.get("sms_activate_country") or "default"
            self.log(f"Entered add_phone, ready to rent phone number: provider={self.provider_key} service={self.service} country={country_label}")
            ...
            try:
                ...
                # If auto-selected country failed, fall back to default country
    ...
    def report_success(self) -> None:
        if self.activation and self.provider and not self.completed:
            ...
            self.phase = "done"
            self.awaiting_external_success = False
            self.log(f"SMS verification successful, marked number as complete: activation_id={self.activation.activation_id}")
        if self._verify_lock_acquired:
            _HERO_SMS_VERIFY_LOCK.release()
            self._verify_lock_acquired = False
