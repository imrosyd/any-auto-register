"""Dynamic proxy IP providers — concrete implementations migrated to providers/proxy/

Supports two modes:
  1. Static proxy: reads fixed proxy list from DB (existing logic)
  2. Dynamic proxy: fetches proxy IPs in real-time from third-party APIs

Dynamic proxy providers are configured via provider_settings; if unconfigured, falls back to static proxy pool.
"""
...
class BaseProxyProvider(ABC):
    """Base class for dynamic proxy providers."""
...
def create_proxy_provider(provider_key: str, config: dict) -> BaseProxyProvider:
    """Create a proxy provider based on provider_key and config."""
    if provider_key == "api_extract":
        api_url = config.get("proxy_api_url", "")
        if not api_url:
            raise RuntimeError("Dynamic proxy has no API URL configured")
    ...
    if provider_key == "rotating_gateway":
        ...
        if not gateway:
            raise RuntimeError("Rotating proxy has no gateway URL configured")
...
def get_dynamic_proxy(extra: dict | None = None) -> Optional[str]:
    """Attempt to get a proxy from the configured dynamic proxy provider.

    Returns None (falls back to static proxy pool) if no dynamic proxy is configured.
    """
    try:
