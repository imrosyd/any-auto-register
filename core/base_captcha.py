"""Base captcha solver — concrete implementations migrated to providers/captcha/"""
...
def create_captcha_solver(provider_key: str, extra: dict | None = None) -> BaseCaptcha:
    ...
    definition = ProviderDefinitionsRepository().get_by_key("captcha", key)
    if not definition or not definition.enabled:
        raise RuntimeError(f"Captcha provider does not exist or is not enabled: {key}")
    merged = ProviderSettingsRepository().resolve_runtime_settings("captcha", key, extra or {})
    ...
    if driver_type == "yescaptcha_api":
        ...
        if not client_key:
            raise RuntimeError("YesCaptcha Key not configured, cannot proceed with protocol registration")
        return YesCaptcha(client_key)
    if driver_type == "twocaptcha_api":
        api_key = str(merged.get("twocaptcha_key", "") or "")
        if not api_key:
            raise RuntimeError("2Captcha Key not configured, cannot proceed with protocol registration")
