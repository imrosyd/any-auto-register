"""
HAR file analysis tool — parses authentication flow and outputs a structured report.
...
Usage:
    ...
    # Show auth-related requests only
...
def format_report(report: dict, format: str = "text") -> str:
    ...
    lines.append("=" * 70)
    lines.append("HAR Authentication Flow Analysis Report")
    lines.append("=" * 70)
...
def main():
    ...
    parser.add_argument("--auth-only", action="store_true", help="Show only auth-related requests")
    parser.add_argument("--json", action="store_true", help="Output JSON format")
    parser.add_argument("--strict", action="store_true", help="Strict mode: show only core auth requests")
    ...
    if args.strict:
        # Strict filter: keep only core auth requests
        NOISE_PATTERNS = {
        ...
        for entry in report["entries"]:
            ...
            if entry["status"] in (301, 302, 303, 307, 308):
                ...
                continue
            # Keep core auth requests
            if any(p in url_lower for p in KEEP_PATTERNS):
                filtered.append(entry)
                continue
            # Keep POST to auth domains
