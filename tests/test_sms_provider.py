...
class TestCreateSmsProvider:
    ...
    def test_sms_activate_missing_key(self):
        with pytest.raises(RuntimeError, match="SMS-Activate API Key not configured"):
    ...
    def test_herosms_missing_key(self):
        with pytest.raises(RuntimeError, match="HeroSMS API Key not configured"):
            create_sms_provider("herosms", {})

    def test_unknown_provider(self):
        with pytest.raises(RuntimeError, match="Unknown SMS provider"):
...
class TestCreatePhoneCallbacks:
    ...
    def test_provider_is_created_lazily_and_cleanup_cancels_pending_activation(self, monkeypatch):
        ...
        assert any("ready to rent phone number" in item for item in logs)
