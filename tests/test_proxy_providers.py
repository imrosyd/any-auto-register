...
class TestCreateProxyProvider:
    ...
    def test_api_extract_missing_url(self):
        with pytest.raises(RuntimeError, match="Dynamic proxy has no API URL configured"):
            create_proxy_provider("api_extract", {})

    def test_rotating_missing_gateway(self):
        with pytest.raises(RuntimeError, match="Rotating proxy has no gateway URL configured"):
            create_proxy_provider("rotating_gateway", {})

    def test_unknown_provider(self):
        with pytest.raises(RuntimeError, match="Unknown proxy provider"):
