...
def mark_incomplete_tasks_interrupted() -> None:
    with Session(engine) as session:
        ...
        for task in tasks:
            ...
            task.error = task.error or "Task was interrupted after service restart"
...
def request_cancel(task_id: str) -> Optional[dict[str, Any]]:
    ...
    if not task:
        return None
    append_task_event(task_id, "Cancel requested for task", event_type="state", level="warning")
    return serialize_task(task)
...
def _request_cancel_mutation(task: TaskModel) -> None:
    ...
    if task.status == TASK_STATUS_PENDING:
        ...
        task.finished_at = _utcnow()
        task.error = task.error or "Task was cancelled before starting"
    else:
...
class TaskLogger:
    ...
    def mark_running(self) -> None:
        ...
        self.log("Task has started executing", event_type="state")
...
def _auto_push_any2api(task_logger: TaskLogger, account) -> None:
    """Auto-push account to Any2API after successful registration (if configured)."""
    try:
        from core.any2api_sync import push_account_to_any2api
        push_account_to_any2api(account, log_fn=task_logger.log)
    except Exception as exc:
        task_logger.log(f"  [Any2API] Auto-push error: {exc}", level="warning")
...
def _auto_upload_cpa(task_logger: TaskLogger, account) -> None:
    ...
    try:
        ...
        task_logger.log(f"  [CPA] Auto-upload error: {exc}", level="warning")
...
def execute_task(task_id: str) -> None:
    ...
    if logger.is_cancel_requested():
        logger.finish(TASK_STATUS_CANCELLED, error="Task was cancelled immediately after starting")
        return
...
def _auto_followup_windsurf_payment(
    *,
    ...
    if use_browser:
        ...
        if turnstile_token:
            params["turnstile_token"] = turnstile_token
    logger.log("Registration successful, starting to auto-generate Windsurf Pro Trial Stripe link")
    try:
...
def _execute_register_task(payload: dict[str, Any], logger: TaskLogger) -> None:
    ...
    if herosms_enabled:
        logger.log(
            ...
            f"when number is still reusable, up to {hero_extra_max} extra successes"
        )
            ...
            try:
                ...
                if alive:
                    logger.log(
                        ...
                        f"used {int(info.get('use_count') or 0)} times"
                    )
                return bool(alive)
            except Exception:
                return False
...
def _execute_account_check_all_task(payload: dict[str, Any], logger: TaskLogger) -> None:
    ...
    for model in accounts:
        ...
        try:
            ...
            results["error"] += 1
            logger.record_error(str(exc))
            logger.log(f"{model.email}: Check error {exc}", level="error")
        completed += 1
