...
class BlinkPlatform(BasePlatform):
    ...
    def _prepare_registration_password(self, password: str | None) -> str | None:
        # blink.new has no password, uses magic link login
        return ""
    ...
    def execute_action(self, action_id: str, account: Account, params: dict) -> dict:
        ...
        if action_id in {"generate_checkout_link", "payment_link", "get_cashier_url"}:
            ...
            if not workspace_id:
                return {"ok": False, "error": "No workspace_id found, cannot generate Blink payment link"}
            ...
            if not url:
                return {"ok": False, "error": "Blink did not return a payment link"}
            return {
                "ok": True,
                "data": {
                    "url": url,
        ...
        if action_id == "create_api_key":
            ...
            if not workspace_id:
                return {"ok": False, "error": "No workspace_id found, cannot create Blink API Key"}
