...
class BlinkProtocolMailboxWorker:
    ...
    def run(
        ...
        *,
        ...
        """Complete registration flow, returns Blink account fields for persistence."""
        ...
        raw = link_callback()
        if not raw:
            raise RuntimeError("Magic link retrieval timed out")

        # otp_callback may return full URL or plain token
        ...
        # Step 4: Firebase sign-in to get idToken
        ...
        # Step 9: Fetch session-data once, save normalized plan/quota summary
        ...
        self.log(
            f"Registration success: {email} workspace={workspace_slug} "
    ...
    def _maybe_create_checkout_link(
        ...
        *,
        ...
        if not workspace_id:
            self.log("Skipping auto-generated payment link: missing workspace_id")
            return "", ""
        if not price_id:
            self.log("Skipping auto-generated payment link: Blink Pro price_id not configured")
            return "", ""
        ...
        try:
            ...
            self.log(f"Auto-generating payment link failed, ignoring and continuing: {exc}")
            return "", ""
    ...
    def _extract_token(raw: str) -> str:
        ...
        if m:
            return m.group(1)
        # If it is directly a 64-digit hex token
        raw = raw.strip()
