...
def register_with_browser_oauth(
    *,
    ...
    with OAuthBrowser(
        ...
        log_fn("Entering Tavily sign-up page")
        ...
        if provider:
            log_fn(f"Switching to {provider_label} sign-in entry")
            if not click_oauth_provider(page, provider):
                page.goto("https://app.tavily.com/sign-in", wait_until="networkidle", timeout=30000)
                time.sleep(2)
                if not click_oauth_provider(page, provider):
                    raise RuntimeError(f"{provider_label} sign-in entry not found")

        method_text = provider_label or "Any available method: email, Google, GitHub, LinkedIn, Microsoft"
        log_fn(f"Please complete sign-in/auth in the browser, you can use {method_text}, max wait {timeout} seconds")
        if email_hint:
            log_fn(f"Please confirm the final login account email: {email_hint}")
        if not wait_for_manual_oauth_completion(page, timeout=timeout):
            raise RuntimeError(f"Tavily browser sign-in did not complete within {timeout} seconds")
