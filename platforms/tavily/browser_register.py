"""Tavily browser registration flow."""
import re
import time
...
class TavilyBrowserRegister:
    ...
    def _solve_turnstile(self, url: str, sitekey: str) -> Optional[str]:
        ...
        try:
            ...
            self.log(f"Turnstile solving failed: {exc}")
            return None

    def _finalize_api_key(self, page) -> str:
        self.log("Extracting and validating Tavily API Key")
        close_marketing_dialog(page)
        ...
        if not verify_api_key(api_key):
            raise RuntimeError("Tavily API Key validation failed")
        return api_key

    def _recover_password_challenge(self, page, password: str, max_attempts: int = 3) -> bool:
        self.log("Password page did not complete redirect, starting security challenge recovery")
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            self.log(f"Password page recovery attempt {attempt}/{max_attempts}")
            self.log(f"  DOM: {format_turnstile_state(state)}")
            if feedback:
                self.log(f"  Page feedback: {feedback}")
            ...
            if has_password_challenge_signal(feedback, state):
                ...
                if token and inject_turnstile_token(page, token):
                    self.log("Password page Turnstile token injected")
    ...
    def run(self, email: str, password: str) -> dict:
        ...
        with Camoufox(**launch_options) as browser:
            ...
            if not signup_url:
                raise RuntimeError("Tavily sign-up entry not found")

            self.log("Entering Tavily sign-up page")
            ...
            if not email_selector:
                raise RuntimeError("Email input field not found on sign-up page")

            self.log("Processing sign-up page Turnstile")
            token = self._solve_turnstile(page.url, get_turnstile_sitekey(page))
            if not token:
                raise RuntimeError("Sign-up page Turnstile solving failed")
            ...
            try:
                ...
                if org_only_message:
                    raise RuntimeError(
                        "Tavily has disabled regular email/password registration, only specific organizations can use Email/password, "
                        "regular accounts need to use Google/GitHub/LinkedIn/Microsoft OAuth."
                    )
                ...
                try:
                    ...
                    feedback = extract_page_feedback(page)
                    raise RuntimeError(f"Did not reach verification code/password page: {feedback or page.url}")

            if page.query_selector('input[name="code"]'):
                if not self.otp_callback:
                    raise RuntimeError("Current flow requires email verification code, but otp_callback was not provided")
            ...
            try:
                ...
                raise RuntimeError(f"Did not reach password page: {page.url}")

            self.log("Setting Tavily password")
            if not self._submit_password_with_recovery(page, password):
                feedback = extract_page_feedback(page)
                raise RuntimeError(f"Password submission failed: {feedback or page.url}")

            time.sleep(3)
            if "verify" in page.url.lower():
                if not self.verification_link_callback:
                    raise RuntimeError("Current flow requires email verification link, but verification_link_callback was not provided")
                self.log("Waiting for Tavily verification link")
                verify_url = self.verification_link_callback()
                if not verify_url:
                    raise RuntimeError("Failed to get Tavily verification link")
