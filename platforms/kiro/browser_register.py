...
def _js_click_by_text(page, *texts) -> bool:
    """Use JS to find the smallest leaf node with exact textContent match and click."""
    for text in texts:
        try:
...
class KiroBrowserRegister:
    ...
    def _handle_aws_profile_spa(self, page, email: str, password: str) -> None:
        ...
        """
        deadline = time.time() + 300  # Wait max 5 minutes for entire flow
        ...
        while time.time() < deadline:
            ...
            # Check if hash is stuck (same hash too long and already handled) -> allow retry
            if hash_part == prev_hash:
                if hash_stuck_since is None:
                    ...
                    if step_key in handled_steps:
                        self.log(f"Step {step_key} stuck for 20 seconds, removing marker to retry")
            ...
            if "enter-email" in hash_part and "enter-email" not in handled_steps:
                ...
                if enter_email_retries > 5:
                    raise RuntimeError(
                        f"AWS enter-email step exceeded 5 retries without progress - "
                ...
                time.sleep(1.5)  # Allow SPA rendering time
                # Fill email (if empty)
                ...
                if not name_filled:
                    self.log("Failed to fill name, trying JS method")
                    try:
                        page.evaluate(f"""
                        () => {{
                ...
                # Wait for hash change (max 12 seconds), if unchanged clear marker and allow retry
                ...
                while time.time() - start_wait < 12:
                    ...
                    # hash unchanged after 12s, submission may have failed, allow retry
                    self.log("enter-email submitted but URL unchanged, will retry")
            ...
            if "enter-name" in hash_part and "enter-name" not in handled_steps:
                self.log("AWS step: Filling name")
                time.sleep(1.5)
                name = _random_name()
                name_filled = False
                name_deadline = time.time() + 15
            ...
            if "verify-email" in hash_part and "verify-email" not in handled_steps:
                self.log("AWS step: Filling verification code")
                ...
                if not self.otp_callback:
                    raise RuntimeError("Kiro registration requires email verification code but no otp_callback provided")
            ...
            if "create-password" in hash_part and "create-password" not in handled_steps:
                self.log("AWS step: Setting password")
                time.sleep(1)
                pwd_fields = []
        ...
        raise RuntimeError(f"AWS Builder ID registration did not complete within the allotted time: {page.url}")
    ...
    def run(self, email: str, password: str) -> dict:
        ...
        with Camoufox(**launch_opts) as browser:
            ...
            while time.time() < deadline_builder and not builder_clicked:
                # Playwright locator exact text match
            ...
            # 3. May have secondary "Sign in" arrow (Kiro tab UI)
            _click_submit_button(page, timeout=5)
            time.sleep(2)
            ...
            # 5. If landed on signin.aws (existing account login page), fill email first
            if AWS_SIGNIN_DOMAIN in page.url:
                ...
                if not _fill_input_wait(page, email_selectors, email, timeout=15):
                    raise RuntimeError(f"Email input field not found: {page.url}")
                time.sleep(0.5)
                _click_submit_button(page, timeout=8)
                time.sleep(3)

            # 6. Wait to enter profile.aws.amazon.com (new account registration flow)
            # AWS redirect may take a while, wait max 60 seconds
            ...
            if AWS_PROFILE_DOMAIN in page.url:
                ...
                self.log("Password input page detected, filling password...")
                pwd_selectors = ['input[type="password"]', 'input[name="password"]']
                _fill_input_wait(page, pwd_selectors, password, timeout=10)
                time.sleep(0.5)
                _click_submit_button(page, timeout=5)
                time.sleep(3)
                # Wait again for profile.aws or kiro.dev after password submission
            ...
            # Step 7: Wait for redirect back to kiro.dev
            self.log("Waiting for redirect back to Kiro...")
