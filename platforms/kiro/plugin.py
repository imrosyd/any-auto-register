...
class KiroPlatform(BasePlatform):
    ...
    def build_protocol_mailbox_adapter(self):
        ...
        return ProtocolMailboxAdapter(
            ...
            otp_spec=OtpSpec(
                wait_message="Waiting for verification code...",
    ...
    def execute_action(self, action_id: str, account: Account, params: dict) -> dict:
        ...
        if action_id == "switch_account":
            ...
            refresh_result = {"ok": False, "message": "Current account does not provide refreshToken/clientId/clientSecret, skipping remote refresh validation"}
            ...
            return {"ok": True, "data": {
                ...
                "quota_note": "Kiro can query subscription, trial, and credit usage via Web Portal, but depends on sessionToken browser session; without session, only token refresh validation results can be returned.",
            }}
            ...
            profile_arn = extra.get("profileArn", "")
            current = read_current_kiro_account() or {}
            refresh_state = {"ok": False, "message": "Current account does not provide refreshToken/clientId/clientSecret, cannot perform remote refresh validation"}
            access_token = extra.get("accessToken", "") or account.token
            if refresh_token and client_id and client_secret:
                ok, result = refresh_kiro_token(refresh_token, client_id, client_secret)
                if ok:
            ...
            return {
                ...
                "data": {
                    ...
                    "desktop_app_state": get_kiro_desktop_state(),
                    "quota_note": "Kiro can query subscription, trial and credits usage via Web Portal, but depends on sessionToken browser session; without session, only token refresh validation results are available.",
                },
            }
