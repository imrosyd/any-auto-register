"""
Kiro account switching - writes token file to ~/.aws/sso/cache/, Kiro IDE auto-detects
Based on kiro-account-manager (Tauri/Rust) switch_kiro_account implementation
"""
...
def _fetch_kiro_portal_user_id(access_token: str, session_token: str) -> str:
    ...
    try:
        ...
        if page.status_code != 200:
            logger.error("Failed to fetch Kiro account/usage page: HTTP %s", page.status_code)
            return ""
...
def get_kiro_portal_state(
    ...
    *,
    ...
    """Query Kiro Web Portal account, plan, and usage info."""
    if not access_token or not session_token:
        return None
    ...
    if not user_id:
        return {
            "available": False,
            "error": "Unable to parse UserId from Kiro Web Portal session",
...
def summarize_kiro_usage(portal_state: dict | None) -> dict | None:
    """Extract Kiro Portal response for direct front-end display."""
    if not portal_state:
        return None
...
def switch_kiro_account(
    ...
    """
    Switch Kiro desktop app account (writes token file, no IDE restart needed).
    ...
    try:
        ...
        return True, "Switch successful, Kiro IDE will automatically use the new account"

    except Exception as e:
        logger.error(f"Failed to switch Kiro account: {e}")
    ...
    try:
        if sys == "Darwin":
            ...
            if os.path.exists(kiro_app):
                ...
                return True, "Kiro IDE has been restarted"
            return True, "Kiro IDE has been closed (app path not found, please start manually)"
        ...
        logger.error(f"Failed to restart Kiro IDE: {e}")
        return False, f"Restart failed: {str(e)}"


def read_current_kiro_account() -> dict | None:
    """Read the current Kiro IDE account token in use"""
