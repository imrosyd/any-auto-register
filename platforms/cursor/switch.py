...
def switch_cursor_account(token: str) -> Tuple[bool, str]:
    """
    Switch Cursor account (writes to storage.json, requires restarting Cursor)
    ...
    try:
        ...
        return True, "Switched successfully, please restart Cursor IDE for the new account to take effect"
    
    except Exception as e:
        logger.error(f"Failed to switch Cursor account: {e}")
    ...
    try:
        if system == "Darwin":  # macOS
            ...
            if os.path.exists(cursor_app):
                ...
                return True, "Cursor IDE has been restarted"
            return True, "Cursor IDE has been closed (app path not found, please start manually)"
        ...
        logger.error(f"Failed to restart Cursor IDE: {e}")
        return False, f"Restart failed: {str(e)}"


def read_current_cursor_account() -> dict | None:
    """Read the current Cursor IDE account token in use"""
    storage_path = _get_cursor_storage_path()
...
def has_cursor_valid_payment_method(token: str) -> bool | None:
    """Check if Cursor has bound a valid payment method."""
    from curl_cffi import requests as curl_req
