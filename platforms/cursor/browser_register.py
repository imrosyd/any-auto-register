...
def _inject_turnstile(page, token: str) -> bool:
    """Inject Turnstile token, compatible with explicit rendering mode (Cursor uses this mode)."""
...
def _is_cf_full_block(page) -> bool:
    """Detect if CF full-page challenge is blocking (distinct from inline Turnstile widget).
    
    Full-page blocking characteristics: page only has CF challenge, no normal form content.
    Inline Turnstile: form displays normally, just has CF iframe embedded.
    """
    try:
        content = page.content().lower()
        # Full-page block keywords (excluding challenges.cloudflare.com as that is the widget script)
        full_block_signals = [
            "just a moment",
        ...
        if any(kw in content for kw in full_block_signals):
            # Also confirm no normal form (form present = just inline widget, not full-page block)
...
def _wait_cf_full_block_clear(page, timeout: int = 120, log_fn=print) -> None:
    """Wait for CF full-page block to disappear, actively click Interstitial Turnstile checkbox.
    
    Two types of CF full-page block:
    1. Interactive Turnstile: shows checkbox, needs clicking
    2. Managed Challenge: no-checkbox passive verification, shows spinner/loading, just wait
    """
    deadline = time.time() + timeout
    warned = False
    clicked = False
    while time.time() < deadline:
        ...
        if not clicked:
            try:
                for frame in page.frames:
                    if "challenges.cloudflare.com" in frame.url:
                        ...
                        if box:
                            ...
                            # Simulate human behavior: wait a few seconds "looking at the page", then slowly move to target
                ...
                pass
            if not clicked:
                # iframe not loaded yet, wait a bit before retrying
                time.sleep(1)
        else:
            # Already clicked, waiting for CF passive verification to complete (Managed Challenge may take longer)
            time.sleep(2)


def _has_turnstile_iframe(page) -> bool:
    """Check if there is a Turnstile iframe (including embedded widget)."""
    try:
        for frame in page.frames:
            if "challenges.cloudflare.com" in frame.url:
                return True
...
def _is_turnstile_modal_visible(page) -> bool:
    """Check if Turnstile challenge is visible (uses body text because iframe loads lazily)."""
    try:
        content = page.content().lower()
        signals = [
...
def _click_turnstile_in_iframe(page, log_fn=print) -> bool:
    ...
    Turnstile iframe uses closed Shadow DOM, JS querySelector cannot access it.
    Use bounding box coordinate clicking instead: checkbox at approximately 1/4 left side of iframe.
    Returns True means clicked (does not mean Turnstile passed).
    """
    # Wait for iframe frame to appear in page.frames list
    ...
    if iframe_el:
        try:
            # Wait for bounding box to be valid (iframe rendering takes time, can not click when height=0)
            box = None
            for _ in range(10):
                ...
                time.sleep(1)
            if box:
                # checkbox at left side of iframe, approximately x=24, y=center
                cx = box["x"] + 24
                cy = box["y"] + box["height"] / 2
                ...
                time.sleep(1.5)
                # If not passed yet, try again slightly to the right
    ...
    # Fallback: click using Playwright in-frame coordinates (relative to frame)
    try:
...
def _handle_turnstile(page, log_fn=print, solve_fn=None, wait_secs: int = 12) -> bool:
    """General Turnstile handling: click checkbox after detecting Turnstile.
     
    Can be called at any stage, such as after form submission or password submission.
    Returns True if Turnstile was detected and handled.
    """
    # Check if Turnstile appears (wait up to wait_secs seconds)
    ...
    log_fn("Turnstile detected, trying to directly click iframe checkbox...")
    ...
    if not solved:
        ...
        if solve_fn:
            ...
            if token:
                ...
                time.sleep(3)
                return True
        log_fn("Auto-solving failed, waiting for manual completion (up to 90 seconds)...")
        dl = time.time() + 90
        while time.time() < dl:
        ...
        if _is_turnstile_modal_visible(page):
            ...
            time.sleep(5)
        # If Turnstile passed but no next step yet, try clicking Continue
...
class CursorBrowserRegister:
    ...
    def _solve_turnstile(self, url: str, sitekey: str) -> Optional[str]:
        ...
        if not self.captcha:
            ...
            return None
        try:
            self.log(f"Calling Captcha Solver to solve ({sitekey[:20]}...)...")
    ...
    def run(self, email: str, password: str = "") -> dict:
        ...
        with Camoufox(**launch_opts) as browser:
            ...
            # Inject override via add_init_script before each page loads to bypass Turnstile's detection
...
(function() {
    ...
    // Also patch PointerEvent (CF detects this too)
...
""")
            ...
            # Must include state (with random nonce) in URL for WorkOS to generate authorization_session_id
            ...
            # Only wait for real CF full-page block (won't be falsely triggered by inline Turnstile widget)
            ...
            try:
                page.wait_for_selector(
                    ...
                    timeout=60000,  # 60s - CF Managed Challenge may take a while
                )
            except Exception:
            ...
            # --- Turnstile handling ---
            # Strategy: click checkbox inside iframe directly in the Camoufox browser
            # External solver is ineffective (it opens its own browser but doesn't submit the form, so it can't see Turnstile)
            ...
            while time.time() < turnstile_deadline:
                ...
                if page.query_selector('input[name="otp"], input[name="code"]'):
                    self.log("Already directly jumped to verification code page, skipping Turnstile")
                    break
                time.sleep(1)

            if has_turnstile:
                self.log("Turnstile detected, trying to directly click iframe checkbox...")
                ...
                if not solved:
                    ...
                    if token:
                        ...
                        self.log("Auto-solving failed, waiting for manual pass (up to 90 seconds)...")
                        dl = time.time() + 90
                        while time.time() < dl:
                            if not _is_turnstile_modal_visible(page):
                                break
                    ...
                    # After clicking, wait for Turnstile to complete
                    time.sleep(3)
                    if _is_turnstile_modal_visible(page):
            ...
            try:
                ...
                self.log("Password setup page detected, filling in password...")
            ...
            # --- Turnstile may appear again after password submit (e.g. "Welcome to Cursor" page) ---
            _handle_turnstile(page, self.log, self._solve_turnstile)

            # --- Check for phone verification page ("Phone number" + "Send verification code") ---
            try:
                phone_input = page.query_selector('input[type="tel"], input[placeholder*="555"], input[autocomplete="tel"]')
                if not phone_input:
                    # Wait a few seconds to see if redirected to phone page
            ...
            if phone_input and phone_input.is_visible():
                if self.phone_callback:
                    ...
                    if phone_number:
                        self.log(f"Phone verification page detected, filling phone: {phone_number[:4]}****")
                        phone_input.click()
                        phone_input.fill(str(phone_number).strip())
                        time.sleep(0.5)
                        _click_continue(page)
                        time.sleep(3)
                        # Wait for SMS code input (6 digits)
                        try:
                            page.wait_for_selector(
                            ...
                            self.log(f"Waiting for SMS verification code failed: {e}")
                else:
                    raise RuntimeError(
                        "Cursor registration requires phone verification, but no phone_callback configured."
                        "Please configure an SMS service in RegisterConfig.extra, or complete phone verification manually."
                    )

            # Wait for verification code input (WorkOS email-verification uses 6 separate boxes)
            ...
            while time.time() < deadline_otp:
                ...
                if "email-verification" in page.url or "verify" in page.url:
                    ...
                    for sel in OTP_SELECTORS[:2]:  # Quickly check first two
                        el = page.query_selector(sel)
                        if el and el.is_visible():
                            otp_input = el
                            break
            ...
            if not otp_input:
                raise RuntimeError(f"Verification code input not found (url={page.url})")
            ...
            # WorkOS 6-box OTP: click first box then type sequentially
            try:
                otp_input.click()
                time.sleep(0.3)
            except Exception:
            ...
            time.sleep(1)
            # WorkOS auto-submits, no need to click Continue; press Enter if not submitted
