"""anything.com protocol mailbox registration worker."""
...
class AnythingProtocolMailboxWorker:
    ...
    def run(
        ...
        *,
        ...
        self.log("Waiting for anything magic link...")
        raw = link_callback()
        if not raw:
            raise RuntimeError("Getting anything magic link timed out")
        ...
        self.log(
            f"anything registration success: {result['email']} "
    ...
    def _resolve_magic_link(self, raw: str) -> str:
        ...
        if "/auth/magic-link" in candidate:
            return candidate
        self.log(f"Received email tracking link, trying to resolve redirect: {candidate[:120]}")
    ...
    def _extract_magic_link(raw: str) -> dict[str, str]:
        ...
        if not match:
            ...
            raise RuntimeError(f"Cannot extract code from anything magic link: {candidate[:200]}")
