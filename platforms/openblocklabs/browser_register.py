...
def _click_turnstile_in_iframe(page, log_fn=print) -> bool:
    ...
    if iframe_el:
        try:
            ...
            log_fn(f"Turnstile coordinate click failed: {exc}")

    try:
        cf_frame.locator("body").click(position={"x": 24, "y": 32}, timeout=5000)
        log_fn("✅ Frame coordinate click successful")
        return True
    except Exception as exc:
        log_fn(f"Frame click failed: {exc}")
...
def _wait_cf_full_block_clear(page, timeout: int = 120, log_fn=print) -> None:
    ...
    while time.time() < deadline:
        if not _is_cf_full_block(page):
            return
        if not warned:
            log_fn("Detected Cloudflare full page block, trying to click verify checkbox...")
            warned = True
        try:
        ...
        if not clicked:
            ...
            time.sleep(2)
    raise RuntimeError(f"Cloudflare full page verification failed: {page.url}")
...
def _handle_turnstile(page, log_fn=print, wait_secs: int = 12) -> bool:
    ...
    if not solved:
        log_fn("⚠️ Auto-click failed, waiting for manual pass (up to 90 seconds)...")
...
def _wait_for_visible_element(page, selectors: list[str], timeout: int = 15, *, require_enabled: bool = False):
    ...
    while time.time() < deadline:
        ...
        if el:
            return el, used_sel
        time.sleep(0.2)
    raise RuntimeError(f"Visible element not found: {' | '.join(selectors)}")
...
def _fill_visible_input(page, selectors: list[str], value: str, label: str, timeout: int = 15) -> str:
    ...
    raise RuntimeError(f"{label} input failed: current={current!r}, expected_len={len(value)}, err={last_error}")
...
def _click_visible_button(page, selectors: list[str], timeout: int = 15) -> str:
    ...
    raise RuntimeError(f"Button click failed: {last_error}")
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if "the requested resource was not found" in body_text or body_text == "not found":
                raise RuntimeError(f"Intermediate page returned Not Found: {page.url}")
        except RuntimeError:
            raise
        except Exception:
            pass
        ...
        if now - last_click_at >= 1.5:
            ...
            if btn_el:
                ...
                try:
                    ...
                    log_fn(f"Detected intermediate page, continuing button click: {used_sel}")
                    last_click_at = now
                    time.sleep(2)
                    continue
                except Exception:
...
def _wait_for_signup_session(page, timeout: int = 30) -> str:
    ...
    while time.time() < deadline:
        ...
        try:
            body_text = (page.locator("body").inner_text() or "").strip().lower()
            if "the requested resource was not found" in body_text or body_text == "not found":
                raise RuntimeError(f"WorkOS session initialization failed, page returned Not Found: {page.url}")
        except RuntimeError:
            raise
        except Exception:
            pass
        time.sleep(0.5)
    raise RuntimeError(f"Waiting for authorization_session_id timed out: {page.url}")
...
""")
            ...
            for attempt in range(2):
                ...
                try:
                    ...
                    time.sleep(2)
            if not session_id:
                raise RuntimeError(str(last_open_error or "Did not obtain authorization_session_id"))
            ...
            self.log(f"Filled email: {used_email_sel}")
            ...
            if pwd_el:
                ...
                self.log(f"Filled password: {used_pwd_sel}")
                used_btn_sel = _click_visible_button(page, btn_selectors)
                self.log(f"Clicked submit button: {used_btn_sel}")
                _advance_to_email_verification(page, btn_selectors, log_fn=self.log, timeout=40)
            else:
                used_btn_sel = _click_visible_button(page, btn_selectors)
                self.log(f"Clicked continue button: {used_btn_sel}")
                ...
                self.log(f"Filled password: {used_pwd_sel}")
                used_btn_sel = _click_visible_button(page, btn_selectors)
                self.log(f"Clicked password page submit button: {used_btn_sel}")
                _advance_to_email_verification(page, btn_selectors, log_fn=self.log, timeout=40)
            ...
            if not self.otp_callback:
                raise RuntimeError("OpenBlockLabs registration requires email verification code but otp_callback not provided")
            ...
            try:
                ...
                self.log(f"Filling verification code failed: {exc}")
            ...
            if not _wait_for_url(page, "dashboard.openblocklabs.com", timeout=60):
                ...
                with open("/tmp/openblocks_fail.html", "w") as f:
                    f.write(page.content())
                raise RuntimeError(f"OpenBlockLabs did not redirect to dashboard after registration: {page.url}")

            wos = _get_wos_session(page, timeout=15)
            if not wos:
                self.log("Did not obtain wos_session, saving screenshot to /tmp/openblocks_fail.png")
                page.screenshot(path="/tmp/openblocks_fail.png")
                with open("/tmp/openblocks_fail.html", "w") as f:
                    f.write(page.content())
                raise RuntimeError("Did not obtain wos-session cookie")
