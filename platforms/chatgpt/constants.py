...
APP_VERSION = "2.0.0"
APP_DESCRIPTION = "Automatic registration system for OpenAI/Codex CLI accounts"
...
# Token acquisition uses Codex CLI client (public client, supports PKCE)
...
# OpenAI page types (used for determining account status)
OPENAI_PAGE_TYPES = {
    "EMAIL_OTP_VERIFICATION": "email_otp_verification",  # Registered account, needs OTP verification
...
# Verification code extraction regex (enhanced)
# Simple match: any 6-digit number
OTP_CODE_SIMPLE_PATTERN = r"(?<!\d)(\d{6})(?!\d)"
# Semantic match: verification code with context (e.g. "code is 123456")
OTP_CODE_SEMANTIC_PATTERN = r'(?:code\s+is|verification\s*code\s*[is]?\s*[:：]?\s*)(\d{6})'
...
MICROSOFT_SCOPES = {
    # Legacy IMAP doesn't require specific scope
    "IMAP_OLD": "",
