    ...
    *,
    ...
    with OAuthBrowser(
        ...
        if chrome_user_data_dir or chrome_cdp_url:
            ...
            log_fn(f"Please complete login/authorization in the browser, using {method_text}, max wait {timeout} seconds")
            if email_hint:
        ...
        if not callback_url:
            raise RuntimeError(f"ChatGPT browser login not completed within {timeout} seconds")
