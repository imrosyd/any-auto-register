...
Current implementation targets the local Electron client 'Codex', by writing to its Chromium Cookies database
to complete best-effort local login state switching.
"""
...
def close_codex_app() -> tuple[bool, str]:
    ...
    try:
        ...
        logger.warning("Failed to close Codex: %s", exc)
        return False, f"Failed to close Codex: {exc}"
...
def restart_codex_app() -> tuple[bool, str]:
    ...
    try:
        ...
        logger.warning("Failed to start Codex: %s", exc)
...
def switch_codex_account(session_token: str = "", cookies: str = "") -> tuple[bool, dict]:
    ...
    try:
        ...
        logger.error("Failed to write Codex Cookies: %s", exc)
        return False, {"error": f"Failed to write Codex Cookies: {exc}"}

    return True, {
        "message": "Written Codex local cookies, preparing to restart desktop",
...
def read_current_codex_account() -> dict:
    ...
    try:
        ...
        logger.warning("Failed to read Codex Cookies: %s", exc)
        return {
            "available": True,
            "cookies_path": cookies_path,
            "error": str(exc),
...
def fetch_chatgpt_account_state(
    *,
    ...
    state = {
        ...
        "quota_note": "ChatGPT does not have a stable quota API, currently returning subscription status and account profile info.",
    }
    ...
    if resolved_access:
        ...
        state["valid"] = False
        state["profile_error"] = "Missing access_token and cannot refresh via session_token"
