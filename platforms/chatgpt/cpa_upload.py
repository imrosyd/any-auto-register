...
def generate_token_json(account) -> dict:
    ...
    # 1) Parse account_id from id_token
    ...
    # expired computed from access_token's exp
    ...
    # 4) fallback: session_token refresh to get new access_token
    if not account_id:
        if session_token:
    ...
    if not account_id:
        logger.warning("[CPA] account_id is empty! CPA upload will fail")
