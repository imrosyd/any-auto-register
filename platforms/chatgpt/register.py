...
class RegistrationResult:
    ...
    logs: list = None
    metadata: dict = None
    source: str = "register"  # 'register' or 'login', distinguishes account source
...
class SignupFormResult:
    """Result of submitting a registration form"""
    success: bool
    page_type: str = ""  # page.type field from response
    is_existing_account: bool = False  # whether the account is already registered
...
class RegistrationEngine:
    """
    Registration engine
    Coordinates email service, OAuth flow, and OpenAI API calls
    """
    ...
    def __init__(
        ...
        Args:
            ...
            task_uuid: task UUID (for database recording)
        """
        ...
        self._is_existing_account: bool = False  # whether this is an already registered account (for auto-login)
    ...
    def _log(self, message: str, level: str = "info"):
        ...
        # Record to database (if associated with a task)
        if self.task_uuid:
            try:
                with get_db() as db:
        ...
        # Record to log system based on level
        if level == "error":
    ...
    def _generate_password(self, length: int = DEFAULT_PASSWORD_LENGTH) -> str:
        ...
        # OpenAI registration page rejects alphanumeric-only passwords at higher probability, adding a symbol bit is more stable.
        specials = ",._!@#"
        if length < 10:
            length = 10
    ...
    def _load_create_account_password_page(self) -> bool:
        """Preload the create-account/password page to obtain page-stage cookies."""
        try:
    ...
    def _create_email(self) -> bool:
        ...
        try:
            ...
            if not self.email_info or "email" not in self.email_info:
                self._log("failed to create mailbox: incomplete return info", "error")
                return False

            self.email = self.email_info["email"]
            self._log(f"successfully created mailbox: {self.email}")
            return True
    ...
    def _submit_signup_form(self, did: str, sen_payload: Optional[SentinelPayload]) -> SignupFormResult:
        """
        Submit registration form (establish session via authorize/continue)

        Returns:
            SignupFormResult: submission result, includes account status determination
        """
        try:
            self._device_id = did
            ...
            try:
                ...
                if is_existing:
                    self._log(f"detected existing account, will switch to login flow")
    ...
    def _register_password(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            for index, password in enumerate(candidates, start=1):
                ...
                if response.status_code == 200:
                    # Parse response to detect existing account
                    try:
                        ...
                        if page_type == OPENAI_PAGE_TYPES.get("EMAIL_OTP_VERIFICATION", "email_otp_verification"):
                            self._log("detected existing account, switching to login flow")
                            self._is_existing_account = True
                    except Exception:
                        pass
    ...
    def _mark_email_as_registered(self):
        """Mark the email as already registered (to prevent repeated attempts)"""
        try:
            with get_db() as db:
                # Check if a record already exists for this email
                existing = crud.get_account_by_email(db, self.email)
                if not existing:
                    # Create a failed record, marking the email as already registered
                    crud.create_account(
                        db,
                        email=self.email,
                        password="",  # empty password means registration was not successful
                        email_service=self.email_service.service_type.value,
                        email_service_id=self.email_info.get("service_id") if self.email_info else None,
                        status="failed",
                        extra_data={"register_failed_reason": "email_already_registered_on_openai"}
                    )
                    self._log(f"marked email {self.email} as already registered in database")
        except Exception as e:
    ...
    def _get_verification_code(self) -> Optional[str]:
        ...
        try:
            ...
            if code:
                ...
                self._log("waiting for verification code timed out", "error")
                return None
    ...
    def _validate_verification_code(self, code: str) -> bool:
        ...
        try:
            ...
            self._log(f"failed to verify code: {e}", "error")
            return False
    ...
    def _create_user_account(self) -> bool:
        ...
        try:
            ...
            self._log(f"generated user info: {user_info['name']}, birthdate: {user_info['birthdate']}")
            ...
            try:
                ...
                self._log(f"client_auth_session_dump error: {e}", "warning")
            ...
            # create_account also requires sentinel token (flow=oauth_create_account)
            if self._device_id:
    ...
    def _acquire_codex_callback(self) -> Optional[str]:
        """
        After registration, complete the full Codex CLI OAuth login flow to obtain a callback URL.
        Uses a new session, goes through authorize → authorize/continue → OTP → callback flow.
        """
        try:
            from .constants import (
            ...
            # 6. If OTP is needed, wait for second verification code
            if page_type == "email_otp_verification":
                ...
                if otp_page == "add_phone":
                    self._log("Codex CLI still requires add_phone, cannot skip", "error")
                    return None

            # 7. Password login needed
            elif page_type in ("login_password", "create_account_password"):
                self._log(f"Codex login submitting password...")
                ...
                self._log(f"Codex login password submission: {pwd_resp.status_code}")
                ...
                if pwd_page == "email_otp_verification" or pwd_page == "email_otp_send":
                    ...
                    self._log(f"Codex login OTP -> page_type={otp_page}")
                    if otp_page == "add_phone":
                        self._log("Codex CLI still requires add_phone", "error")
                        return None
    ...
    def _get_workspace_id(self) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if not workspace_id:
                    self._log("unable to parse workspace_id", "error")
                    return None
    ...
    def _select_workspace(self, workspace_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code != 200:
                self._log(f"workspace selection failed: {response.status_code}", "error")
            ...
            self._log(f"workspace selection failed: {e}", "error")
            return None
    ...
    def _follow_redirects(self, start_url: str) -> Optional[str]:
        ...
        try:
            ...
            for i in range(max_redirects):
                ...
                # If not a redirect status code, stop
                if response.status_code not in [301, 302, 303, 307, 308]:
                ...
                # Check if it contains callback parameters
                if "code=" in next_url and "state=" in next_url:
    ...
    def run(self) -> RegistrationResult:
        ...
        - Steps skipped for existing accounts: set password, send verification code, create user account
        ...
        try:
            ...
            # 1. Check IP location
            self._log("1. checking IP location...")
            ip_ok, location = self._check_ip_location()
            if not ip_ok:
                result.error_message = f"IP location not supported: {location}"
            ...
            if self._is_existing_account:
                self._log("8. [existing account] skipping password setup, OTP already sent automatically")
            else:
            ...
            if self._is_existing_account:
                self._log("9. [existing account] skipping verification code send, using auto-sent OTP")
                # For existing accounts, OTP was auto-sent when submitting the form, record timestamp
                self._otp_sent_at = time.time()
            else:
            ...
            self._log("11. verifying verification code...")
            if not self._validate_verification_code(code):
                result.error_message = "failed to verify verification code"
                return result

            # 12. Determine next step based on OTP response
            ...
            if session_token:
                self._log(f"obtained session-token: {session_token[:30]}...")
            if account_cookie:
                self._log(f"obtained _account: {account_cookie}")
            ...
            try:
                ...
                # Use fresh session (Hydra needs clean session)
                login_client = OpenAIHTTPClient(proxy_url=self.proxy_url)
                login_session = login_client.session

                # Visit Codex OAuth URL, follow redirect to /log-in
                ...
                if page_type in ("email_otp_send", "email_otp_verification"):
                    ...
                    code = self._get_verification_code()
                    if not code:
                        raise RuntimeError("Codex OTP retrieval failed")
                    ...
                    if codex_callback:
                        ...
                        self._log(f"Codex token succeeded: keys={list(codex_token_info.keys())}")
                    else:
            ...
            if session_token:
                ...
                self._log(f"obtained Session Token")
    ...
    def save_to_database(self, result: RegistrationResult) -> bool:
        ...
        return True  # database saving is handled uniformly by account_manager
