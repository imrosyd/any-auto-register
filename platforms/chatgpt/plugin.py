...
def _assert_complete_oauth_callback(result) -> None:
    ...
    # Legacy Codex CLI flow returns all 4 fields
...
def _generate_chatgpt_registration_password(length: int = 16) -> str:
    """Generate a password that passes OpenAI registration page validation more reliably.

    Legacy protocol flow has been verified: including lowercase, digit, and symbol significantly improves success rate.
    Also add an uppercase character to avoid browser flow generating passwords that are "long enough but not strong enough."
    """
    specials = ",._!@#"
    minimum_length = 12
