...
def _parse_phone_country_and_local(phone_number: str) -> tuple[str, str, str]:
    """Parse (dial_code, local_number, country_name) from a full phone number.
...
def _select_phone_country_ui(page, dial_code: str, country_name: str, log) -> bool:
    """Select the corresponding country in the add-phone page country dropdown.

    OpenAI add-phone page uses React Aria Select component, which has a hidden native <select> underneath
    and a visible button trigger + listbox popup layer.
    """
    if not dial_code and not country_name:
        log("  unable to identify country code, skipping country selection")
        return False

    iso_code = PHONE_DIAL_TO_ISO.get(dial_code, "")
    log(f"  target country: {country_name} (+{dial_code}) ISO={iso_code}")

    # First check if the dropdown is already on the target country
    ...
    if already:
        log(f"  country already at target value: (+{dial_code})")
        return True

    # ═══════════════════════════════════════════════════════════════════
    # Strategy 1: Set value directly via the underlying native <select> (most reliable)
    # React Aria Select has a hidden <select> underneath for form submission and accessibility.
    # Directly modify its value and trigger change event to sync React state.
    # ═══════════════════════════════════════════════════════════════════
    native_selected = page.evaluate(
        ...
        ({ isoCode, dialCode, countryName }) => {
          ...
          for (const sel of selects) {
            if (sel.options.length < 10) continue;  // exclude non-country select elements
    ...
    if native_selected and native_selected.get("ok"):
        log(f"  ✓ native <select> selection succeeded: value={native_selected.get('value')}")
        time.sleep(0.5)
        # Verify UI sync update
    ...
    # ═══════════════════════════════════════════════════════════════════
    # Strategy 2: Operate directly via React Aria key attribute
    # ═══════════════════════════════════════════════════════════════════
    key_selected = page.evaluate(
        """
        ({ isoCode, dialCode, countryName }) => {
          // find the hidden <select> of React Aria Select and simulate via selectOption
          const selects = document.querySelectorAll('select');
          for (const sel of selects) {
    ...
    try:
        ...
        if select_el:
            # Try selecting by ISO code
            if iso_code:
                try:
                    select_el.select_option(value=iso_code)
                    log(f"  ✓ Playwright selectOption(value={iso_code}) succeeded")
                    time.sleep(0.5)
                    return True
                except Exception:
                    pass
            # Try matching by label (contains country name or dial code)
            try:
                # Get all option values and text, find a match
                ...
                if match_value:
                    ...
                    log(f"  ✓ Playwright selectOption(value={match_value}) succeeded")
                    time.sleep(0.5)
                    return True
            except Exception as e:
                log(f"  selectOption label match failed: {e}")
    except Exception as e:
    ...
    if not trigger:
        ...
        if not trigger:
            ...
            return False
        log("  triggered button via JS click")
    else:
        trigger.scroll_into_view_if_needed()
        trigger.click()
        log("  clicked country selector dropdown")
    ...
    # Find and click the target option in listbox
    option = None
    if iso_code:
    ...
    if option:
        ...
        log(f"  dropdown displays after selection: {new_text}")
        if f"+{dial_code}" in (new_text or ""):
            log(f"  ✓ country selection succeeded: {new_text}")
            return True

    # Keyboard type-ahead search
    log(f"  trying keyboard type-ahead: {country_name}")
    ...
    if f"+{dial_code}" in (final_text or ""):
        log(f"  ✓ type-ahead selection succeeded: {final_text}")
        return True
...
def _click_passwordless_login_if_available(page, log, *, context: str) -> bool:
    ...
    if selector:
        log(f"{context} selected passwordless login: {selector}")
        time.sleep(1)
        return True
    try:
        clicked = bool(
        ...
        clicked = False
    if clicked:
        log(f"{context} selected passwordless login")
        time.sleep(1)
    return clicked
...
def _recover_signup_password_page(page, log) -> bool:
    ...
    if not selector:
        return False
    log(f"password page landed on login state, trying registration entry recovery: {selector}")
    time.sleep(1.2)
    return True
...
def _wait_for_signup_entry_transition(page, log, timeout: int = 20) -> dict:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.25)
    raise RuntimeError("did not navigate to password/verification page after email submission")


def _start_browser_signup_via_page(page, email: str, log) -> dict:
    for entry_url in (PLATFORM_LOGIN_ENTRY, f"{OPENAI_AUTH}/log-in"):
        try:
            log(f"opening OpenAI registration entry: {entry_url}")
        ...
        if submit_selector:
            ...
            log("email page no clickable Continue found, using form fallback submission")
        else:
            raise RuntimeError("email page no Continue button found")

        return _wait_for_signup_entry_transition(page, log)

    raise RuntimeError("no OpenAI registration entry email input found")
...
def _start_browser_signup_via_authorize(page, email: str, device_id: str, log) -> dict:
    ...
    if not authorize_url:
        raise RuntimeError("email submission failed, no authorize URL obtained")
...
def _complete_oauth_with_session(cookies_dict: dict, oauth_start, proxy: str | None, log) -> dict | None:
    ...
    try:
        ...
        log(f"  selecting workspace: {workspace_id}")
        ws_resp = s.post(
        ...
        if orgs and orgs[0].get("id"):
            ...
            log(f"  selecting organization: {org_id}")
            org_resp = s.post(
        ...
        if not callback_url:
            log("  ⚠️ could not follow OAuth callback")
            return None
        result_json = submit_callback_url(
        ...
        log(f"  OAuth session completion error: {e}")
        return None
...
def _submit_login_email_via_page(page, email: str, log) -> dict:
    ...
    if not input_selector:
        raise RuntimeError("OAuth email page no input found")
    ...
    if submit_selector:
        ...
        log("OAuth email page no clickable Continue found, using form fallback submission")
    else:
        raise RuntimeError("OAuth email page no Continue button found")
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "OAuth email page did not redirect after submission"}
...
def _do_codex_oauth(page, cookies_dict: dict, email: str, password: str, otp_callback, phone_callback, proxy: str | None, log) -> dict | None:
    """Complete Codex OAuth in a real browser session, returning the full token package."""
    ...
    try:
        ...
        for step in range(20):
            ...
            if (
                ...
                log("  OAuth page detected updated authorization link, following...")
                page.goto(page_oauth_url, wait_until="domcontentloaded", timeout=30000)
                continue

            if state["page_type"] == "login_email":
                log("  OAuth page needs email login, submitting email...")
            ...
            if state["page_type"] in {"login_password", "create_account_password"}:
                log("  OAuth page needs password login, submitting password...")
                # fill password directly in OAuth flow, do not attempt to restore to registration state
            ...
            if state["page_type"] == "email_otp_verification":
                if not otp_callback:
                    log("  ⚠️ OAuth needs email OTP but no otp_callback provided")
                    return None
                log("  OAuth waiting for email verification code...")
                code = otp_callback()
                if not code:
                    log("  ⚠️ OAuth OTP retrieval failed")
                    return None
            ...
            if state["page_type"] == "about_you":
                log("  OAuth page shows about_you, continuing form fill...")
            ...
            if state["page_type"] == "add_phone":
                ...
                # first try to skip add_phone, re-visit OAuth authorization URL
                # user already logged in, re-visiting auth URL should go directly to callback
                ...
                try:
                    ...
                    # checking if callback was obtained directly
                    callback_url = ""
                    ...
                    if callback_url:
                        log("  ✓ successfully skipped add_phone, obtained OAuth callback")
                        return _submit_callback_result(callback_url, oauth_start, proxy)
                    ...
                    if skip_state.get("page_type") in {"consent", "workspace_selection", "organization_selection"}:
                        log("  ✓ skipped add_phone, reached consent page")
                        # trying to complete consent flow in browser
                        browser_result = _complete_oauth_in_browser(page, oauth_start, proxy, log)
                        if browser_result:
                            return browser_result
                        # falling back to curl session approach
                        cookies_dict = _get_cookies(page)
                        session_result = _complete_oauth_with_session(cookies_dict, oauth_start, proxy, log)
                        if session_result:
                            return session_result
                    ...
                    if callback_url:
                        return _submit_callback_result(callback_url, oauth_start, proxy)
                    log(f"  skip add_phone error: {exc}")
            ...
            # waiting longer for redirect to complete
            if state["page_type"] == "chatgpt_home":
                # checking if error page
...
def _handle_post_signup_onboarding(page, log) -> None:
    ...
    if "chatgpt.com" not in current_url:
        return
    try:
        # may get persistent storage prompt, prefer Allow, Block also ok for main flow.
    ...
    # common onboarding survey for new accounts, prefer Skip.
    try:
...
def _handle_add_phone_challenge(
    ...
    *,
    ...
    """Complete phone number verification in the add-phone page via UI interaction.

    Flow: select country -> enter local number -> click send -> fill OTP -> click verify.
    If verification code times out, automatically retry with a different number (up to max_phone_attempts times).
    """
    if not phone_callback:
        raise RuntimeError(
            "ChatGPT registration encountered phone verification but phone_callback is not configured."
            "Configure SMS service in RegisterConfig.extra, or complete phone verification manually."
        )

    last_error = None
    for phone_attempt in range(max_phone_attempts):
        if phone_attempt > 0:
            log(f"retry with new number attempt {phone_attempt + 1}/{max_phone_attempts} ...")
            # returning to add-phone page
            try:
        ...
        try:
            ...
            last_error = exc
            error_msg = str(exc)
            # retry new number on OTP timeout or used number, throw other errors directly
            should_retry = (
                "SMS verification code not obtained" in error_msg
            ...
            if not should_retry:
                raise
            log(f"⚠️ verification code timed out, preparing to retry with new number...")
            # canceling current number
            if hasattr(phone_callback, "cleanup"):
                phone_callback.cleanup()
            # resetting phone_callback state to need_number
    ...
    raise last_error or RuntimeError("SMS verification failed: no code received after multiple retries")
...
def _do_add_phone_attempt(
    ...
    *,
    ...
    """Single phone verification attempt (internal function)."""

    # preserving HTTP resend callback for internal SMS provider use
    ...
    def _request_openai_resend():
        # in browser mode, only click Resend via page UI
        ...
        if resend_clicked:
            ...
            log("  phone-otp/resend -> page Resend button not found, skipping (browser mode does not use HTTP)")

    if hasattr(phone_callback, "set_resend_callback"):
        phone_callback.set_resend_callback(_request_openai_resend)

    # ---- Step 1: Get phone number ----
    log("registration flow entered add_phone, preparing to rent number and receive SMS...")
    phone_number = str(phone_callback() or "").strip()
    if not phone_number:
        raise RuntimeError("no phone number obtained")
    log(f"detected add_phone, submitting phone number (UI): {_mask_phone_number(phone_number)}")

    # parsing country dial code and local number
    dial_code, local_number, country_name = _parse_phone_country_and_local(phone_number)
    log(f"  parsed number: country={country_name or 'unknown'} dial_code=+{dial_code} local={local_number[:4]}...")

    # ensuring on add-phone page
    current_url = str(page.url or "")
    ...
    if phone_input_sel:
        # if country selected successfully, enter local number; otherwise enter full number
        ...
        # _fill_input_like_user uses strict equality, but add-phone page may auto-prepend country prefix to input
        # so additionally check if input.value contains our entered number
        if not filled:
            try:
                ...
                # if input value contains our number (may have +56 prefix etc.), consider success
                if fill_value and fill_value in actual_val.replace(" ", "").replace("-", ""):
                    filled = True
                    log(f"  phone filled (with prefix): {actual_val[:12]}...")
            except Exception:
                pass
        if not filled:
            # fallback: try clearing first then use keyboard.type to enter
            log(f"  _fill_input_like_user failed, trying keyboard fallback...")
            try:
                page.click(phone_input_sel)
                time.sleep(0.3)
                # triple select-all delete to ensure cleared
                for _ in range(3):
                ...
                if fill_value in actual_clean:
                    filled = True
                    log(f"  keyboard fallback succeeded: {str(actual or '')[:12]}...")
            except Exception as e:
                log(f"  keyboard fallback failed: {e}")
        if not filled:
            # final fallback: directly set value via JS
            try:
                js_ok = page.evaluate(
                    """
                    ({ selector, value }) => {
                ...
                if js_ok:
                    filled = True
                    log(f"  JS setValue fallback succeeded")
            except Exception as e:
                log(f"  JS setValue fallback failed: {e}")
        if not filled:
            raise RuntimeError(f"phone input fill failed: {phone_input_sel}")
        log(f"  phone input already filled: {phone_input_sel} value={fill_value[:4]}...")
    else:
        raise RuntimeError("no phone input found")
    _browser_pause(page)
    ...
    if send_sel:
        ...
        log("  send button not found, using form fallback submission")
    else:
        raise RuntimeError("no send verification code button found")

    # waiting for page response (may show OTP input or error)
    time.sleep(2)

    # check if send succeeded (page should show OTP input or URL change)
    error_text = _extract_auth_error_text(page)
    if error_text:
    ...
    log("phone submitted successfully (UI), waiting for SMS code...")

    # ---- Step 5: Wait for SMS code and fill in page OTP input ----
    for code_attempt in range(3):
        sms_code = str(phone_callback() or "").strip()
        if not sms_code:
            raise RuntimeError("no SMS verification code obtained")

        # waiting for OTP input to appear
        otp_sel = _wait_for_any_selector(page, OTP_INPUT_SELECTORS, timeout=10)
        if not otp_sel:
            # trying phone input selectors as OTP (some versions reuse same input)
            otp_sel = _find_first_selector(page, PHONE_INPUT_SELECTORS)
        if not otp_sel:
            raise RuntimeError("no SMS verification code input found")

        # using same fill logic as email OTP
        ...
        # checking if invalid verification code
        ...
        if page_error and any(kw in page_error.lower() for kw in ("invalid", "incorrect", "wrong", "expired")):
            log(f"SMS code deemed invalid: {page_error[:100]}，waiting for next code...")
    ...
    raise RuntimeError("SMS verification failed: all codes invalid or did not pass")
...
def _complete_oauth_in_browser(page, oauth_start, proxy, log) -> dict | None:
    """Complete OAuth consent flow in browser with multi-strategy retry clicking Continue.

    Based on Chrome extension project step9 implementation:
    ...
    def _try_extract_callback(url: str) -> dict | None:
        ...
        try:
            ...
            # when state is missing or mismatched, if URL is indeed our callback, skip state validation and exchange token directly
            if "state" in str(ve) and "localhost" in url and "code=" in url:
                try:
                    # manually extract code, skip state validation
    ...
    def _wait_for_callback(timeout_sec: int) -> dict | None:
        ...
        while time.time() < deadline:
            ...
            if result:
                return result
            # also check for navigation to localhost (even if page load fails)
    ...
    def _find_consent_button():
        ...
        if btn:
            return btn
        # Strategy 2: Global search for Continue button
        for sel in [
    ...
    def _click_strategy_request_submit(log_round: int) -> bool:
        ...
        try:
            ...
            log(f"  consent requestSubmit error: {e}")
            return False
    ...
    try:
        ...
        try:
            ...
            if try_again and try_again.is_visible():
                log("  consent page error, clicking Try again...")
                try_again.click()
                time.sleep(3)
        except Exception:
            pass
        ...
        for round_idx in range(MAX_ROUNDS):
            ...
            if clicked:
                # consent submission will redirect to localhost:1455/auth/callback
                # since no local service is listening, browser may show connection error, but URL is already updated
                try:
                    page.wait_for_url("**/auth/callback*", timeout=15000)
                except Exception:
                    pass  # ignore timeout or navigation errors, URL will be checked below
                time.sleep(1)
                result = _wait_for_callback(CLICK_EFFECT_TIMEOUT)
                if result:
                    log("  ✓ browser OAuth consent completed")
                    return result
                log(f"  consent round {round_idx + 1} click did not redirect")
            else:
                log(f"  consent round {round_idx + 1} no button found")

            # refresh page before last round retry
            if round_idx < MAX_ROUNDS - 1:
                log(f"  consent refreshing page for round {round_idx + 2}...")
                try:
        ...
        log(f"  consent {MAX_ROUNDS} rounds attempted, not yet complete, current: {str(page.url or '')[:100]}")
        return None
    except Exception as exc:
        ...
        if cb:
            ...
            if result:
                log("  ✓ extracted callback from exception to complete OAuth")
                return result
        log(f"  browser OAuth consent error: {exc}")
        return None


def _submit_oauth_password_direct(page, password: str, log) -> dict:
    """OAuth flow specific: fill password directly for login, do not attempt to restore registration state."""
    input_selector = _wait_for_any_selector(page, PASSWORD_INPUT_SELECTORS, timeout=15)
    if not input_selector:
        # password input did not appear, page may still be loading or has redirected
        # wait and retry
        time.sleep(2)
        input_selector = _wait_for_any_selector(page, PASSWORD_INPUT_SELECTORS, timeout=10)
    if not input_selector:
        raise RuntimeError("OAuth password page no input found")
    ...
    if submit_selector:
        ...
        log("  OAuth password page using form fallback submission")
    else:
        raise RuntimeError("OAuth password page no Continue button found")
    ...
    while time.time() < deadline:
        ...
        if error_text:
            return {"ok": False, "status": 400, "url": current_url, "data": None, "text": error_text}
        time.sleep(0.5)
    return {"ok": False, "status": 0, "url": str(page.url or ""), "data": None, "text": "OAuth password did not redirect after submission"}
...
def _submit_password_via_page(page, password: str, log) -> dict:
    ...
    input_selector = _wait_for_any_selector(page, PASSWORD_INPUT_SELECTORS, timeout=15)
    if not input_selector:
        raise RuntimeError("password page no input found")
    if not _fill_input_like_user(page, input_selector, password):
    ...
    if submit_selector:
        ...
        log("password page no clickable Continue found, using form fallback submission")
    else:
        raise RuntimeError("password page no Continue button found")
    ...
    while time.time() < deadline:
        ...
        if page_type == "login_password" and _recover_signup_password_page(page, log):
            ...
            if not input_selector:
                return {"ok": False, "status": 400, "url": current_url, "data": None, "text": "login password page restored but no registration password input found"}
            if not _fill_input_like_user(page, input_selector, password):
                return {"ok": False, "status": 400, "url": current_url, "data": None, "text": "login password page restored but password re-fill failed"}
            submit_selector = _click_first(page, PASSWORD_SUBMIT_SELECTORS, timeout=5)
            if submit_selector:
                log(f"restored, re-clicking password submit button: {submit_selector}")
                start_url = str(page.url or start_url)
                time.sleep(0.4)
                continue
            if _submit_form_with_fallback(page, input_selector):
                log("restored, submit button not found, using form fallback submission")
                start_url = str(page.url or start_url)
                time.sleep(0.4)
                continue
            return {"ok": False, "status": 400, "url": current_url, "data": None, "text": "login password page restored but no submission method found"}
        error_text = _extract_auth_error_text(page)
        if error_text:
        ...
        time.sleep(0.5)
    _dump_debug(page, "chatgpt_password_fail")
    return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "password page did not redirect after submission"}
...
def _submit_otp_via_page(page, code: str, log) -> dict:
    ...
    # wait for page load, ensure OTP input is rendered
    try:
    ...
    try:
        ...
        if count >= len(otp):
            ...
            if done >= len(otp):
                filled = True
                log(f"verification code page filled {done}  segmented inputs")
    except Exception:
        pass

    # then try single input
    if not filled:
        otp_candidates = [
        ...
        for candidate in otp_candidates:
            try:
                ...
                if final_value:
                    filled = True
                    log("verification code page filled single input")
                    break
            except Exception:
                continue

    if not filled:
        # wait 3s and retry (page may still be rendering)
        time.sleep(3)
        otp_retry_selectors = [
            "input[inputmode='numeric']",
            "input[autocomplete='one-time-code']",
            ...
            try:
                ...
                if target.is_visible(timeout=2000):
                    ...
                    if str(target.input_value() or "").strip():
                        filled = True
                        log("verification page filled single input (retry)")
                        break
            except Exception:
                continue

    if not filled:
        return {"ok": False, "status": 0, "url": page.url, "data": None, "text": "verification code page no fillable input found"}
    ...
    if not submit_selector:
        return {"ok": False, "status": 0, "url": page.url, "data": None, "text": "verification code page no Continue button found"}
    ...
    while time.time() < deadline:
        ...
        if error_text:
            return {"ok": False, "status": 400, "url": current_url, "data": None, "text": error_text}
        time.sleep(0.5)
    return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "verification code page did not redirect after submission"}
...
def _submit_about_you_via_page(page, log) -> dict:
    ...
    def _fill_second_visible_input(values: list[str], excluded_visible_indices: set[int] | None = None) -> bool:
        """Fallback: about_you card typically has Full name + Birthday/Age two inputs."""
        try:
    ...
    def _fill_birthday_selects(yyyy: str, mm: str, dd: str) -> bool:
        """Handle Month/Day/Year dropdown style birthday control."""
        try:
    ...
    if about_mode == "age":
        log(
            "about_you age direct locate: "
            f"name={direct_name_selector or '-'}, age={direct_age_selector or '-'}"
        )

    def _fill_segmented_date(mm: str, dd: str, yyyy: str) -> bool:
        """Handle MM / DD / YYYY segmented date input (React DateField style).
        Feature: multiple small inputs or div[data-type] segments under a Birthday label."""
        try:
            # Method 1: div[data-type] segments (React Aria DateField)
            ...
            # click input, then sequentially enter MM DD YYYY (Tab between segments)
            ...
            # Method 3: second visible input under Birthday label, click then type digits
            ...
            if inputs.count() >= 2:
                ...
                page.keyboard.press("Backspace")
                time.sleep(0.1)
                # enter MM, Tab to DD, Tab to YYYY
                ...
                time.sleep(0.3)
                # verify correct values were entered
    ...
    if about_mode == "birthday_select":
            ...
            if not fill_result.get("age") and len(ordered_visible_entries) < 2:
                for candidate in age_candidates:
                    if _fill_locator(candidate, str(age_years)):
                        fill_result["age"] = True
                        break
        # fallback: directly find placeholder="Age" input
        ...
        # first try segmented date input (MM / DD / YYYY DateField)
    ...
    if not fill_result.get("name"):
        raise RuntimeError("about_you did not successfully fill Full name")
    if not (
        fill_result.get("birthdate")
        or fill_result.get("age")
        or (fill_result.get("month") and fill_result.get("day") and fill_result.get("year"))
    ):
        raise RuntimeError("about_you did not successfully fill Birthday/Age")
    _browser_pause(page)
    ...
    if not submit_selector:
        raise RuntimeError("about_you no submit button found")
    ...
    while time.time() < deadline:
        ...
        if error_text and "oai_log" not in error_text and "SSR_HTML" not in error_text:
            ...
            if (
                ...
                log("about_you age mode submission rejected, re-syncing Full name/Age/hidden birthday and retrying...")
                if direct_name_selector and _fill_input_like_user(page, direct_name_selector, name):
                    fill_result["name"] = True
                elif _fill_visible_input_entry(name_entry, name):
                    fill_result["name"] = True
    ...
    return {"ok": False, "status": 0, "url": last_url, "data": None, "text": "about_you did not redirect after submission"}
...
def _browser_registration_flow(page, email: str, password: str, otp_callback, phone_callback, log) -> dict:
    ...
    log(
        "authorized state cookies: "
        f"login_session={'yes' if auth_cookies.get('login_session') else 'no'}, "
        f"oai-did={'yes' if auth_cookies.get('oai-did') else 'no'}"
    )
    log(f"registration state start: page={state.get('page_type') or '-'} url={(state.get('current_url') or '')[:100]}")
    ...
    for step in range(12):
        ...
        log(
            f"registration state progress: step={step+1} page={state.get('page_type') or '-'} "
        ...
        if _is_password_registration(state):
            if register_submitted:
                raise RuntimeError("re-entered password registration stage")
            ...
            log(f"password page submission status: {reg_resp.get('status', 0)}")
            if not reg_resp.get("ok"):
                raise RuntimeError(f"password page submission failed: {(reg_resp.get('text') or '')[:300]}")
        ...
        if str(state.get("page_type") or "") == "login_password":
            if _recover_signup_password_page(page, log):
                state = _derive_registration_state_from_page(page)
                continue
            log("registration flow landed on existing account login password page, continuing authentication via login flow...")
            login_resp = _submit_oauth_password_direct(page, password, log)
            log(f"login password page status: {login_resp.get('status', 0)}")
            if not login_resp.get("ok"):
                raise RuntimeError(f"login password page failed: {(login_resp.get('text') or '')[:300]}")
        ...
        if _is_email_otp(state):
            if not otp_callback:
                raise RuntimeError("ChatGPT registration needs email verification code but no otp_callback provided")
            log("waiting for ChatGPT verification code")
            code = otp_callback()
            if not code:
                raise RuntimeError("no verification code obtained")
            otp_resp = _submit_otp_via_page(page, code, log)
            log(f"verification page submission status: {otp_resp.get('status', 0)}")
            if not otp_resp.get("ok"):
        ...
        if _is_about_you(state):
            ...
            if "about-you" not in str(page.url):
                log(f"redirected to about_you page: {target_url[:120]}")
                page.goto(target_url, wait_until="domcontentloaded", timeout=30000)
            about_resp = _submit_about_you_via_page(page, log)
            log(f"about_you submission status: {about_resp.get('status', 0)}")
            if not about_resp.get("ok"):
                raise RuntimeError(f"about_you submission failed: {(about_resp.get('text') or '')[:300]}")
    ...
    raise RuntimeError("registration state machine exceeded max steps")
...
class ChatGPTBrowserRegister:
    ...
    def run(self, email: str, password: str) -> dict:
        ...
        with Camoufox(**launch_opts) as browser:
            ...
            self.log(f"registration flow complete: page={final_state.get('page_type') or '-'}")

            # getting session token and cookies
            cookies_dict = _get_cookies(page)

            # ═══ Obtaining correct token via Codex CLI OAuth ═══
            # browser context session state after registration is unstable (NS_BINDING_ABORTED),
            # using a fresh browser for OAuth is more reliable
            self.log("executing Codex CLI OAuth flow to get token...")

        # directly use fresh browser for OAuth (registration browser context unreliable)
        ...
        raise RuntimeError("ChatGPT registration did not complete full OAuth callback, falling back to session/access_token semi-finished result is rejected")

    def _retry_oauth_fresh_browser(self, email, password):
        """Perform Codex OAuth in a fresh browser context (bypassing add_phone session)."""
        proxy = _build_proxy_config(self.proxy)
        launch_opts = {"headless": self.headless}
        if proxy:
            launch_opts["proxy"] = proxy
        try:
            ...
            self.log(f"  fresh browser OAuth error: {e}")
            return None
