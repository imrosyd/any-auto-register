...
Supports Session Token and OAuth Refresh Token refresh methods
"""
...
class TokenRefreshManager:
    ...
    def refresh_by_session_token(self, session_token: str) -> TokenRefreshResult:
        ...
        try:
            ...
            logger.info(f"Session token refresh successful, expires at: {expires_at}")
            return result
    ...
    def refresh_by_oauth_token(
        ...
        try:
            ...
            logger.info(f"OAuth token refresh successful, expires at: {expires_at}")
            return result
    ...
    def refresh_account(self, account: Account) -> TokenRefreshResult:
        ...
        if account.session_token:
            logger.info(f"Attempting to refresh account {account.email} using Session Token")
            result = self.refresh_by_session_token(account.session_token)
            if result.success:
                return result
            logger.warning(f"Session Token refresh failed, attempting OAuth refresh")

        # Try OAuth Refresh Token
        if account.refresh_token:
            logger.info(f"Attempting to refresh account {account.email} using OAuth Refresh Token")
