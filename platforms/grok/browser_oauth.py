    ...
    *,
    ...
    with OAuthBrowser(
        ...
        if chrome_user_data_dir or chrome_cdp_url:
            ...
            log_fn(f"Please complete login in the browser using {method_text}, max wait {timeout} seconds")
            if email_hint:
        ...
        if not sso:
            raise RuntimeError(f"Grok browser login did not get SSO Cookie within {timeout} seconds")
