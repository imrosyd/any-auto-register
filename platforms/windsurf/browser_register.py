            ...
            for locator in (
                ...
                try:
                    if locator.count() and locator.is_visible() and locator.is_enabled():
                        ...
                        log_fn("Clicked Windsurf Turnstile popup Continue")
                        return True
                except Exception:
                    pass
        try:
...
def _wait_cf_full_block_clear(page: Page, timeout: int = 120, log_fn: Callable[[str], None] = print) -> None:
    ...
    while time.time() < deadline:
        if not _is_cf_full_block(page):
            break
        if not warned:
            log_fn("Detected Windsurf Cloudflare full page block, trying to click verify checkbox...")
            warned = True
        try:
...
def _handle_turnstile(
    ...
    *,
    ...
    if _is_cf_full_block(page):
        ...
        if "checkout.stripe.com" in str(page.url or ""):
            return True
    log_fn("Detected Windsurf Turnstile, trying direct iframe checkbox click...")
    ...
    if solved:
        ...
        if _is_turnstile_modal_visible(page):
            log_fn("Windsurf Turnstile still showing, continuing to wait for auto-pass...")
            time.sleep(5)
...
class WindsurfBrowserRegister:
    ...
    def _run_with_page(self, page: Page, *, email: str, password: str, name: str) -> dict:
        ...
        self.log("Step0: Opening Windsurf login page and entering registration")
        ...
        if not auth_data and auth_token:
            self.log("Page did not capture WindsurfPostAuth response, using page session to supplement exchange")
        ...
        if not auth_data.get("session_token"):
            ...
            try:
                ...
                pass
            raise RuntimeError(f"Windsurf page registration did not get session_token, current page: {visible_text}")
        auth = auth_data
        ...
        self.log(
            f"Windsurf browser registration success: {email} "
    ...
    def _extract_code(raw: str) -> str:
        ...
        raise RuntimeError(f"Cannot extract Windsurf 6-digit verification code from email: {str(raw or '')[:200]}")
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                if token:
                    return token
                time.sleep(1)
            raise TimeoutError("Waiting for browser Turnstile token timed out")
        finally:
            context.close()
            browser.close()
...
class WindsurfStripeCheckoutBrowser:
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                if not final_url:
                    ...
                    raise RuntimeError(f"Alipay authorization link not obtained, current page: {visible_text}")
    ...
    def _complete_alipay_checkout(
        ...
        *,
        ...
        page.on("response", _capture_response)
        try:
            self.log("Step5: Trying to select Alipay and fill billing info")
            ...
            self.log("Step6: Submitting Stripe Checkout, getting Alipay authorization link")
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if "alipay" in body_text.lower() and ("scan" in body_text.lower() or "qr" in body_text.lower() or "authorize" in body_text.lower()):
                        current_url = str(page.url or "").strip()
                        if current_url and not self._is_stripe_checkout_url(current_url):
                            final_url = current_url
                            self.log(f"Step7: Current page shows Alipay scan/authorization content: {final_url[:160]}")
                            break
                except Exception:
                    pass
                page.wait_for_timeout(1000)
            ...
            if not final_url:
                ...
                current_url = str(page.url or "").strip()
                raise RuntimeError(
                    "Did not enter Alipay scan/authorization page, cannot save Stripe Checkout page."
    ...
    def _fill_checkout_form(
        ...
        *,
        ...
        self._fill_if_empty(
            page,
            [re.compile(r"address line 1|street address", re.I)],
            billing_line1,
            selectors=(
                '#billingAddressLine1',
        ...
        self._fill_if_empty(
            page,
            [re.compile(r"address line 2|apartment|suite", re.I)],
            billing_line2,
            selectors=(
                '#billingAddressLine2',
    ...
    def _select_alipay(self, page: Page) -> None:
        ...
        for target in (
            ...
            try:
                ...
                pass
        raise RuntimeError(f"Stripe Checkout: Alipay payment method not found or cannot be selected, visible methods: {self._visible_payment_methods(page)}")
    ...
    def _wait_for_billing_fields(self, page: Page, *, timeout: int = 12000) -> bool:
        ...
        raise RuntimeError("Alipay selected but billing form did not appear")
    ...
    def _submit_checkout(self, page: Page) -> None:
                ...
                if button.count():
                    ...
                    if button.is_enabled():
                        ...
                        if not ready and "SubmitButton--incomplete" in initial_state["class_name"]:
                            self.log(f"Step6: Stripe submit button still marked incomplete but clickable, proceeding: {self._submit_button_state(page)}")
                        ...
                        if "SubmitButton--incomplete" in initial_state["class_name"]:
                            try:
                                ...
                                if refreshed.count() and refreshed.is_visible() and refreshed.is_enabled():
                                    ...
                                    self.log("Step6: Re-clicked after Stripe submit button became complete")
                            except Exception:
                                self.log(f"Step6: Waiting for Stripe submit button to become complete timed out: {self._submit_button_state(page)}")
                        return
            except Exception:
                pass
        ...
        for button in reversed(buttons):
            try:
                ...
                pass
        raise RuntimeError("Stripe Checkout: clickable submit button not found")
    ...
    def _check_checkout_boxes(self, page: Page) -> None:
        try:
            ...
            pass
        patterns = (re.compile(r"terms|agree|authorize|mandate", re.I),)
        for pattern in patterns:
            try:
            ...
            for locator in (
                ...
                try:
                    if locator.count() and locator.is_visible() and locator.is_enabled():
                        ...
                        self.log("Step6: Clicked Alipay redirect page continue button")
                        return True
                except Exception:
                    pass
        try:
...
class WindsurfBrowserPaymentFlow:
        ...
        *,
        ...
        if not token and Camoufox is not None:
            try:
                ...
                self.log(f"Camoufox generation of Windsurf Pro Trial Stripe link failed, falling back to Playwright: {exc}")
        with sync_playwright() as pw:
            launch_opts = {
                "headless": self.headless,
                "args": ["--disable-blink-features=AutomationControlled", "--disable-dev-shm-usage", "--no-sandbox"],
    ...
    def run(
        ...
        *,
        ...
        if not token and Camoufox is not None:
            try:
                self.log("Step0: Using Camoufox to attempt Windsurf Turnstile bypass")
                checkout_url = WindsurfCamoufoxCheckoutFlow(
                    headless=self.headless,
                    proxy=self.proxy,
                    log_fn=self.log,
                ...
                self.log(f"Camoufox entry to Windsurf Pro failed, falling back to Playwright flow: {exc}")
        with sync_playwright() as pw:
            launch_opts = {
                "headless": self.headless,
                "args": ["--disable-blink-features=AutomationControlled", "--disable-dev-shm-usage", "--no-sandbox"],
    ...
    def _open_pro_checkout(self, page: Page, *, turnstile_token: str, timeout: int) -> str:
        ...
        token = str(turnstile_token or "").strip()
        if not token:
            raise RuntimeError("Page still on pricing after clicking Pro, in-page Turnstile handling did not reach Stripe, and no turnstile_token fallback")
        self.log("Step4: Using Turnstile token to proceed to Pro Checkout")
        ...
        raise RuntimeError(f"Entering Stripe Checkout timed out, current page: {visible_text}")
