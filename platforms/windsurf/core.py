...
Windsurf website mainly uses two types of interfaces in HAR:
  1. /_devin-auth/* JSON interfaces for email verification code registration/login
  2. /_backend/exa.* application/proto interfaces for querying user, plan and quota

This does not rely on full protobuf generated code, only implements lightweight
...
class WindsurfClient:
    ...
    def login_with_password(self, email: str, password: str) -> dict[str, str]:
        """Login to existing account with email+password, return session_token etc."""
