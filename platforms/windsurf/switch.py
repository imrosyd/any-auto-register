"""
Windsurf desktop app account switch - pure protocol implementation
Supports macOS / Windows / Linux

Switch flow (no browser needed, no Electron safeStorage manipulation):
...
3. Windsurf internally uses OTT to authenticate and switch accounts

Windsurf auth info is cached in state.vscdb SQLite database:
...
def _read_db_key(db_path: str, key: str) -> str | None:
    """Read the value of a specified key from state.vscdb"""
...
def _write_db_key(db_path: str, key: str, value: str):
    """Write the value of a specified key to state.vscdb (INSERT OR REPLACE)"""
...
def _clear_old_auth_keys(db_path: str):
    """Clear encrypted session and auth cache of old account in state.vscdb"""
    if not os.path.exists(db_path):
        return
    ...
    try:
        ...
        logger.warning(f"Failed to clear old auth key (non-fatal): {e}")


def _get_one_time_auth_token(session_token: str, proxy: str | None = None) -> str:
    """Call GetOneTimeAuthToken with session_token to get one-time auth token"""
...
def switch_windsurf_account(
    *,
    ...
    Flow:
    1. Call GetOneTimeAuthToken API with session_token -> get OTT
    2. Pass to Windsurf via windsurf:// deep link -> complete auth switch
    ...
    try:
        ...
        logger.info(f"OTT acquired successfully: {ott[:20]}...")

        if not _open_deep_link(ott):
            return False, "OTT acquired but cannot open deep link, please open Windsurf manually"
...
def restart_windsurf_ide() -> Tuple[bool, str]:
    ...
    try:
        if system == "Darwin":
            ...
            return True, "Windsurf IDE closed (application path not found, please launch manually)"
...
def read_current_windsurf_account() -> dict | None:
    """Read current Windsurf IDE account info"""
