...
function getBackendPath() {
  if (isDev) {
    return null // Dev mode: manually start uvicorn
  }
  // Production mode: PyInstaller executable placed in resources/backend/
...
function waitForBackend(retries = 180, onProgress = null) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const attempt = (n) => {
      ...
      if (backendExited) {
        reject(new Error('Backend process has exited, check logs'))
        return
      }
      ...
      http.get(`http://localhost:${PORT}/api/health`, (res) => {
        ...
        else reject(new Error('Backend startup timeout (180s), check firewall or port usage'))
      }).on('error', () => {
