...
class ApiExtractProvider(BaseProxyProvider):
    """Generic API extract mode — calls a URL to return a proxy IP list.

    Suitable for most proxy providers' "API extract" interfaces, return format is usually:
    ...
    def from_config(cls, config: dict) -> 'ApiExtractProvider':
        ...
        if not api_url:
            raise RuntimeError("Dynamic proxy not configured with API URL")
        return cls(
            api_url=api_url,
