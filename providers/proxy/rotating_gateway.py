...
class RotatingProxyProvider(BaseProxyProvider):
    """Fixed-entry rotating proxy — automatically assigns a different IP per request.

    Suitable for providers offering a fixed gateway address (e.g., BrightData, Oxylabs, IPRoyal, etc.),
    format is usually: http://user:pass@gate.provider.com:port
    Each request through this gateway automatically uses a different exit IP.
    """
    ...
    def from_config(cls, config: dict) -> 'RotatingProxyProvider':
        ...
        if not gateway:
            raise RuntimeError("Rotating proxy not configured with gateway address")
