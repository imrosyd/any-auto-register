...
_VERSION_CACHE: dict = {}
_VERSION_CACHE_TTL = 600  # 10 min — stay under GitHub API rate limit (60/h unauthenticated)
_VERSION_CACHE_LOCK = threading.Lock()


def _fetch_latest_release() -> dict | None:
    """Fetch the latest GitHub release with a 10-min cache."""
    now = time.time()
...
def _is_newer(latest: str, current: str) -> bool:
    """Compare semver strings; fall back to string comparison if either fails to parse."""
...
@router.get("/version")
def get_version():
    """Return the current version and the latest GitHub release. The frontend uses this to decide whether to show an update prompt."""
    current = __version__
