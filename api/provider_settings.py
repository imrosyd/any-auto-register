...
def test_provider(body: ProviderTestRequest):
    """Test whether a provider config is correct — tries to create/fetch a mailbox address."""
    ...
    if body.provider_type == "mailbox":
        ...
        return {"ok": True, "message": "Captcha services do not support online testing; verify in a registration task instead"}
    elif body.provider_type == "sms":
        return {"ok": True, "message": "SMS services do not support online testing; verify in a registration task instead"}
    else:
        return {"ok": False, "error": f"Testing not supported for provider type: {body.provider_type}"}


def _test_mailbox(driver_type: str, extra: dict, definition) -> dict:
    """Try to create a mailbox with the given config to verify it is correct."""
    import traceback
    ...
    try:
        ...
        if hasattr(mailbox, "peek_email"):
            ...
            return {
                "ok": True,
                "message": f"Test successful! Available mailbox: {email}",
                "email": email,
            }
        ...
        return {
            "ok": True,
            "message": f"Test successful! Generated mailbox: {account.email}",
            "email": account.email,
        }
