...
export default function Register() {
  ...
  const activeTaskStats = task ? [
    ...
    { label: 'Succeeded', value: String(task.success ?? 0), icon: CheckCircle },
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_340px]">
        <div className="space-y-4">
              ...
              {form.identity_provider === 'oauth_browser' && (
                <>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    For background-browser automation of third-party accounts, configure a Chrome Profile or Chrome CDP first to reuse the browser session already signed in on this machine.
                  </p>
                </>
              )}
            </CardContent>
          ...
          {form.identity_provider === 'mailbox' && (
            <Card>
              <CardHeader><CardTitle>System mailbox config</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                ...
                {mailboxProviderOptions.length > 0 ? (
                  ...
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                    No mailbox provider is enabled. Go to Settings to add and enable a default mailbox provider first.
                  </div>
                )}
          ...
          {smsProviderOptions.length > 0 && (
            <Card>
              <CardHeader><CardTitle>SMS verification config</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
                    {optionsError}
        ...
        <div className="space-y-5 xl:sticky xl:top-4 xl:self-start">
          ...
          {task ? (
            <>
                ...
                <CardContent className="space-y-4">
                  ...
                  {task.status === 'interrupted' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">Task was interrupted after a service restart</span>
                    </div>
                  )}
              ...
              <Card>
                <CardHeader><CardTitle>Real-time log</CardTitle></CardHeader>
                <CardContent>
            ...
            <Card className="bg-[var(--bg-pane)]/55">
              <CardHeader><CardTitle>Waiting to run</CardTitle></CardHeader>
              <CardContent className="text-sm text-[var(--text-secondary)]">Status and logs appear after creation.</CardContent>
            </Card>
          )}
        </div>
      </div>
