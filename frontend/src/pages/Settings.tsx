...
const PROVIDER_META: Record<ProviderType, {
  ...
  mailbox: {
    ...
    createTitle: 'New dynamic mailbox provider',
    addDialogHint: 'Pick from the mailbox provider catalog',
    usageHint: 'These mailbox settings are only used when the registration identity is "System mailbox". You can view details, edit, set default and delete inline in the list.',
    ...
    listDescription: (count: number) => `${count} configs — view details, edit, set default, delete.`,
    noAvailableText: 'No more mailbox providers to add',
    availableText: (count: number) => `${count} more mailbox provider(s) available to add`,
    emptyText: 'No mailbox provider configured yet. Add one first.',
  ...
  captcha: {
    ...
    createTitle: 'New dynamic captcha provider',
    addDialogHint: 'Pick from the captcha provider catalog',
    usageHint: 'Protocol mode auto-selects remote captcha services in enabled order; browser mode uses the current default captcha provider. You can view details, edit, set default and delete inline in the list.',
    ...
    listDescription: (count: number) => `${count} configs — protocol mode reads the available ones here in order.`,
    noAvailableText: 'No more captcha providers to add',
    availableText: (count: number) => `${count} more captcha provider(s) available to add`,
    emptyText: 'No captcha provider configured yet. Add one first.',
  ...
  sms: {
    ...
    createTitle: 'New dynamic SMS provider',
    addDialogHint: 'Pick from the SMS provider catalog',
    usageHint: 'When a platform requires phone verification, the enabled SMS provider here is used to create a temporary number and fill in the SMS code. You can view details, edit, set default and delete inline in the list.',
    ...
    listDescription: (count: number) => `${count} configs — phone fill-in and SMS verification prefer the default here.`,
    noAvailableText: 'No more SMS providers to add',
    availableText: (count: number) => `${count} more SMS provider(s) available to add`,
    emptyText: 'No SMS provider configured yet. Add one first.',
...
const TABS: { id: string; label: string; icon: any; sections?: any[] }[] = [
  {
    ...
    sections: [{
      ...
      desc: 'These are the default behaviors; the account list and register page reuse them directly.',
      items: [
        ...
        { key: 'default_executor', label: 'Default execution mode' },
      ],
    }, {
      section: 'Browser reuse',
      desc: 'Background-browser automation of third-party accounts usually needs to reuse the browser already signed in on this machine.',
      items: [
  ...
  },
  {
    ...
    sections: [{
      section: 'CPA panel',
      desc: 'Auto-upload to the CPA management platform after registration',
      items: [
      ...
      <div className="dialog-panel dialog-panel-md flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          {item.fields.length === 0 ? (
            <div className="text-sm text-[var(--text-muted)] py-3">This provider needs no extra configuration.</div>
          ) : (
    ...
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-md flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <div>
            <h2 className="text-base font-semibold text-[var(--text-primary)]">{title}</h2>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Add a dynamic provider definition and create its first usable config at the same time.</p>
          </div>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X className="h-4 w-4" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          {currentFields.length === 0 ? (
            <div className="text-sm text-[var(--text-muted)] py-3">This driver family needs no extra config fields.</div>
          ) : (
            <GroupedProviderFields
...
export default function Settings({ embedded, defaultTab }: { embedded?: boolean; defaultTab?: string }) {
  ...
  const loadConfigData = async () => {
    ...
    if (options) {
      ...
      setOptionsError('Provider metadata not loaded. Restart the backend and refresh the page.')
    }
  }
  ...
  const getSelectOptions = (key: string) => {
    ...
    if (key === 'default_oauth_provider') {
      return [
        { label: 'No preset, choose on the current page', value: '' },
  ...
  const saveProviderSetting = async (providerType: ProviderType, item: ProviderSetting) => {
    ...
    try {
      ...
      setProviderNotice(current => ({ ...current, [providerType]: `Saved config for ${item.catalog_label || item.provider_key}` }))
  ...
  const createProviderDefinitionAndSetting = async (providerType: ProviderType) => {
    ...
    if (!payload.provider_key || !payload.label || !payload.driver_type) {
      setProviderError(current => ({ ...current, [providerType]: 'Fill in the provider name, key and driver family first' }))
      return
    }
      ...
      {!embedded && (
        <Card className="overflow-hidden p-2.5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-sm font-semibold text-[var(--text-primary)]">Config</div>
        ...
        <div className="space-y-4">
          {activeTab === 'platform_caps' ? (
            ...
            <>
              {activeTab === 'register' && (
                <div className="rounded-lg border border-[var(--accent-edge)] bg-[var(--accent-soft)] px-4 py-3 text-sm text-[var(--text-secondary)]">
                  Regular users only need to understand two things: choose "System mailbox" or "Third-party account" for the registration identity, and choose "Protocol mode / Background browser automation / Visible browser automation" for the execution mode. The settings here only set the defaults.
                </div>
              )}
