        ...
        <Card className="bg-[var(--bg-pane)]/60">
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-[var(--border-soft)] bg-[var(--bg-pane)]/45 px-3.5 py-2.5 text-xs leading-5 text-[var(--text-secondary)]">
              Enter a single proxy, or paste multiple lines to bulk import. The region tag is stored alongside for later filtering and endpoint identification.
            </div>
            ...
            {proxies.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8">
                  <div className="empty-state-panel">The proxy pool is empty. Add one on the left or bulk import to get started.</div>
                </td>
              </tr>
            )}
            {proxies.map(p => (
