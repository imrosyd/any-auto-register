    ...
    try {
      ...
      if (selection.identityProvider === 'mailbox') {
        if (!defaultMailboxProvider?.provider_key) {
          throw new Error('No default mailbox provider configured. Enable a mailbox provider in Settings first.')
        }
    ...
    <div className="dialog-backdrop" onClick={!taskId ? onClose : undefined}>
      <div className="dialog-panel dialog-panel-md flex flex-col"
        ...
        <div className="px-6 py-4 flex-1 overflow-y-auto flex flex-col gap-5">
          {!taskId ? (
            configLoading ? (
              <div className="text-sm text-[var(--text-muted)]">Loading registration config...</div>
            ) : (
              <>
                <div>
                  ...
                  <div className="mt-1 text-xs text-[var(--text-muted)]">Shows whatever the current platform supports — no need to study the platform capability config first.</div>
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">Choose execution mode</div>
                  <div className="mt-1 text-xs text-[var(--text-muted)]">All modes run automatically; they only differ in the protocol or browser channel.</div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-[var(--text-muted)] block mb-1">Count</label>
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 text-xs text-[var(--text-secondary)]">
                  <div>Identity: <span className="text-[var(--text-primary)]">{selectedRegistration?.label || '-'}</span></div>
                  <div className="mt-1">Executor: <span className="text-[var(--text-primary)]">{selectedExecutor?.label || '-'}</span></div>
                  ...
                  {selection.identityProvider === 'oauth_browser' && !reusableBrowser && (
                    <div className="mt-2 text-amber-400">Background browser automation requires a Chrome Profile or Chrome CDP; without it, only visible browser automation is allowed.</div>
                  )}
                </div>
...
function ActionResultHighlights({ payload }: { payload: any }) {
  ...
  if (typeof payload.has_valid_payment_method === 'boolean') stats.push({ label: 'Card linked', value: payload.has_valid_payment_method })
        ...
        <div className="relative overflow-hidden border-b border-[var(--border)] px-6 py-5">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-[var(--text-muted)]">Task status, error summary and real-time logs in one place</p>
            </div>
            <div className="flex items-center gap-2">
              {taskStatus ? (
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Fill in the parameters needed to run this action</p>
          </div>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]">
            <X className="h-4 w-4" />
          </button>
            ...
            <div className="space-y-2">
              ...
              {verificationMailbox?.email && (
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                  Verification mailbox: {verificationMailbox.email} · {verificationMailbox.provider || '-'} · ID {verificationMailbox.account_id || '-'}
                </div>
              )}
            </div>
          )}
...
function ImportModal({ platform, onClose, onDone }: { platform: string; onClose: () => void; onDone: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setResult(`Imported ${res.created} successfully`); onDone()
      ...
      {open && (
        <div className="absolute right-0 top-10 z-20 min-w-[148px] rounded-lg border border-[var(--border)] bg-[var(--bg-card)] py-1 shadow-lg">
          <div className="px-3 py-1 text-[11px] text-[var(--text-muted)]">
            {hasSelection ? `Export ${selectedIds.length} selected accounts` : 'Export current filtered results'}
          </div>
          {options.map(option => (
            <button
              key={option.key}
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-2.5 bg-[var(--bg-pane)]/20">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              title="Refresh all account quotas in one click"
              onClick={async () => {
            ...
            >
              ...
              {batchRefreshing ? 'Refreshing...' : 'Refresh quota'}
            </Button>
            ...
            {selectedCount > 0 && (
              <Button
                ...
                onClick={async () => {
                  if (!confirm(`Delete the ${selectedCount} selected accounts? This cannot be undone.`)) return
              ...
              <tr>
                <td colSpan={7} className="px-4 py-24 text-center">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    <p className="text-xs text-[var(--text-muted)] max-w-sm">No account records found for this platform. You can add one manually or bulk import from a file.</p>
                  </div>
                </td>
              </tr>
            )}
                ...
                <td className="px-3 py-2.5 font-mono text-sm text-[var(--text-primary)] align-top">
                  ...
                  {verificationMailbox && (verificationMailbox.email || verificationMailbox.account_id || verificationMailbox.provider) && (
                    <div
                      ...
                      title={`Verification mailbox: ${verificationMailbox.email || '-'} · ${verificationMailbox.provider || '-'}`}
                    >
