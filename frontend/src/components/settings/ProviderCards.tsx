...
const CATEGORY_GROUPS = [
  ...
  { key: 'selfhost', label: 'Requires self-hosting', desc: 'You deploy the backend service yourself' },
  { key: 'thirdparty', label: 'Third-party services', desc: 'Sign up on a third-party platform to obtain credentials' },
  { key: 'custom', label: 'Custom', desc: 'Connect any API through the generic HTTP driver' },
]
...
function EditModal({
  ...
  // Load options for async-select fields
  useEffect(() => {
    for (const field of fields) {
    ...
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {fields.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)]">This service needs no extra configuration.</p>
          ) : fields.map(field => {
    ...
    <>
      <div className="space-y-6">
        {CATEGORY_GROUPS.map(({ key: cat, label, desc }) => {
          ...
          // Hide the generic HTTP mailbox from the list — it's the engine behind custom providers
