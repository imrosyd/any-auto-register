...
export function getCaptchaStrategyLabel(executorType: string, policy?: CaptchaPolicy, providers?: ProviderOption[]) {
  if (executorType === 'headless' || executorType === 'headed') {
    ...
    return label ? `Browser mode uses ${label} by default` : 'No default captcha provider configured for browser mode'
  }
  const order = policy?.protocol_order || []
  if (order.length === 0) {
    return 'No remote captcha provider configured for protocol mode'
  }
  const labels = order.map(value => providers?.find(item => item.value === value)?.label || value)
  return `Protocol mode auto-selects remote captcha services in enabled order: ${labels.join(' -> ')}`
}
